...
enum ISABELA_status
isabelaDeflateDouble(struct isabela_stream* ib_stream, enum ISABELA_flush ib_flush)
{
    ...
    double *sorted_window_buffer = 0;
    double *approximate_window_buffer = 0;
    ...
    sorted_window_buffer        = (double *) malloc (sizeof (double) * ib_stream->config->window_size);
    approximate_window_buffer   = (double *) malloc (sizeof (double) * ib_stream->config->window_size);
    ...
    for (window_idx = 0; window_idx < nwindows; window_idx ++) {
        ...
        // B-Spline or Wavelet compression
        if (ib_stream->config->transform == ISABELA_BSPLINES) {
            compress_bspline_double   (sorted_window_buffer, current_window_size, (double *) out_buffer_uc, ib_stream->config->ncoefficients);
            compressed_buffer_size = ib_stream->config->ncoefficients * sizeof (double);
        } else if (ib_stream->config->transform == ISABELA_WAVELETS) {
            compress_wavelets_double (sorted_window_buffer, current_window_size, ib_stream->config->ncoefficients, out_buffer_uc, &compressed_buffer_size);
        }
        
        // Build the error coefficients
        if (ib_stream->config->error_rate != 0) {
            if (ib_stream->config->transform == ISABELA_BSPLINES) {
                decompress_bspline_double (approximate_window_buffer, current_window_size, (double *) out_buffer_uc, ib_stream->config->ncoefficients);
            } else {
                uint32_t wavelet_compressed_buffer_size = 0;
                decompress_wavelets_double (approximate_window_buffer, current_window_size, out_buffer_uc, &wavelet_compressed_buffer_size);
...
enum ISABELA_status
isabelaInflateDouble(struct isabela_stream* ib_stream, enum ISABELA_flush ib_flush)
{
    ...
    double *original_window_buffer = 0;
    double *approximate_window_buffer = 0;
    uint32_t *permuted_index = 0;
    void *error_tmp_buffer;
    ...
    for (window_idx = 0; window_idx < nwindows; window_idx ++) {
        ...
        if (ib_stream->config->transform == ISABELA_BSPLINES) {
            decompress_bspline_double (approximate_window_buffer, current_window_size, (double *) out_buffer_uc, ib_stream->config->ncoefficients);
            out_buffer_uc           += ib_stream->config->ncoefficients * sizeof (double);
        } else if (ib_stream->config->transform == ISABELA_WAVELETS) {
            uint32_t wavelet_compressed_buffer_size = 0;
            decompress_wavelets_double (approximate_window_buffer, current_window_size, out_buffer_uc, &wavelet_compressed_buffer_size);
        ...
        for (i = 0; i < current_window_size; i ++) {
            ...
            ((double *) in_buffer_uc) [permuted_index [i]] = approximate_window_buffer [i];
        }

        in_buffer_uc = in_buffer_uc + sizeof (double) * current_window_size;
        ib_stream->avail_out += sizeof (double) * current_window_size;
    }
