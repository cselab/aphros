...
protected:
	...
	void _ic(G& grid)
	{
		const bool load_restartfile = parser.check("-restart");

		if (load_restartfile)
		{
		// READ A COMPRESSED RESTART FILE
		...
		{
			const string restart_status = path + "/restart_status";
		...
		ReadZBin_MPI <G, StreamerDummy_HDF5raw> (grid, datafile.str().c_str(), path.c_str());

		}
		else
		{
		// READ COMPRESSED SIMULATION FILES
		...
		for (int channel = 0; channel < NCHANNELS; channel++)
		{
			...
			suffix << ".StreamerGridPointIterative.channel" << channel;
      
			string fullname = file.str() + suffix.str();
		  
			//cout << "inputfile_name: "  << fullname << "\n" ;
      
			vpreader [channel] = new Reader_WaveletCompressionMPI (mycomm, fullname, swapbytes, wtype_read);
