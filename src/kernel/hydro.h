...
void Hydro<M>::CalcSurfaceTension(const FieldCell<Scal>& fcvf,
  ...
  if (st == "div") { // divergence of tensor (Hu,Adam 2001)
    ...
    if (var.Int.Exists("interrepulse") && var.Int["interrepulse"]) {
      ...
      for (auto c : ll) {
        for (auto cc : ll) {
          if (c != cc) {
            ...
            auto dx = xcc - xc;
            auto r = dx.norm();
            //if ((xc[1] - th) * (xcc[1] - th) < 0) {
            if (1) {
              auto v = dx / (r * r *r);
              for (auto q : m.Nci(c)) {
                auto f = m.GetNeighbourFace(c, q);
                ffbp_[f] += k * v.dot(m.GetNormal(f)) * (p[cc] - p[cc]);
              }
              for (auto q : m.Nci(cc)) {
                auto f = m.GetNeighbourFace(cc, q);
                ffbp_[f] += -k * v.dot(m.GetNormal(f)) * (p[c] - p[cc]);
              }
            }
          }
        }
      ...
      */
      ...
      for (auto c : m.Cells()) {
        if (b.IsInside(m.GetCenter(c))) {
          for (auto q : m.Nci(c)) {
            auto f = m.GetNeighbourFace(c, q);
            ffbp_[f] += k * m.GetOutwardFactor(c, q);
          }
        }
      }
      */
