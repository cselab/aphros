...
void EV() {
  ...
  class ExplVisc {
   ...
   public:
    ExplVisc(Mesh& m) : m(m), fcv(m), fcf(m), ffmu(m) {
  ...
  auto m = geom::InitUniformMesh<Mesh>(dom, b, s, hl);
  ...
  std::cout << "******** T(ev) = " << st.GetSeconds() << std::endl;
  std::cout << "bc.size()=" << m.GetBlockCells().size() << std::endl;
  std::cout << "ibc.size()=" << m.GetInBlockCells().size() << std::endl;
  std::cout << "bf.size()=" << m.GetBlockFaces().size() << std::endl;
  std::cout << "ibf.size()=" << m.GetInBlockFaces().size() << std::endl;
