// Created by Petr Karnakov on 29.07.2022
...
FieldFace<typename M::Scal> GetPositiveAreaFraction(
  ...
  if (M::dim == 2) {
    for (auto f : m.Faces()) {
      const IdxNode n = m.GetNode(f, 0);
      const IdxNode np = m.GetNode(f, 1);
      ffs[f] = GetPositiveEdgeFraction(fnl[n], fnl[np]);
    }
  } else {
    fassert_equal(M::dim, 3);
    for (auto f : m.Faces()) {
...
std::function<void(FieldCell<typename M::Scal>&, const M&)> CreateInitU(
  ...
  if (v == "circle") {
    ...
    return [](FieldCell<Scal>& fc, const M& m) { fc.Reinit(m, 0); };
  } else if (v == "uniform") {
