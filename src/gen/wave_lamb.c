...
c = 1/tanh(h*k);
eta = a*sq(e)
       *((3.0E+0*(sq(sq(c)-1)+8*pow(c,6))*cos(3*k*x))/6.4E+1
        +((-3.0E+0)*((-3*sq(c))+pow(c,4)+3)*cos(k*x))/8.0E+0)
    +(a*c*(3*sq(c)-1)*e*cos(2*k*x))/4.0E+0+a*cos(k*x);
o = sqrt(g*k*tanh(h*k))*sqrt(sq(a)*sq(k)
                                  *((9.0E+0*sq(1/pow(tanh(h*k),2)-1))/8.0E+0
                                   +1/pow(tanh(h*k),2))
                              +1);
vx = (3.0E+0*a*(sq(c)-1)*(sq(c)+3)*(9*sq(c)-13)*sq(e)*g*k*cos(3*k*x)
            *cosh(3*k*(y+h)))
   /(6.4E+1*cosh(3*h*k)*o)
   +(3.0E+0*a*sq(sq(c)-1)*e*g*k*cos(2*k*x)*cosh(2*k*(y+h)))
    /(4.0E+0*c*cosh(2*h*k)*o)+(a*g*k*cos(k*x)*cosh(k*(y+h)))/(cosh(h*k)*o);
vy = (3.0E+0*a*(sq(c)-1)*(sq(c)+3)*(9*sq(c)-13)*sq(e)*g*k*sin(3*k*x)
	    *sinh(3*k*(y+h)))
   /(6.4E+1*cosh(3*h*k)*o)
   +(3.0E+0*a*sq(sq(c)-1)*e*g*k*sin(2*k*x)*sinh(2*k*(y+h)))
    /(4.0E+0*c*cosh(2*h*k)*o)+(a*g*k*sin(k*x)*sinh(k*(y+h)))/(cosh(h*k)*o);
...
void
f1(double x, double y, double *pvx, double *pvy, double *peta)
{
...
a*pow(eps, 2)*((1.0L/64.0L)*(24*pow(chi, 6) + 3*pow(pow(chi,
2) - 1, 2))*cos(3*k*x) + (1.0L/8.0L)*(-3*pow(chi, 4) +
9*pow(chi, 2) - 9)*cos(k*x)) + a*cos(k*x);

omega = sqrt(g*k*(pow(eps, 2)*(pow(chi, 2) +
(9.0L/8.0L)*pow(pow(chi, 2) - 1, 2)) + 1)*tanh(h*k));

vx = (3.0L/64.0L)*a*pow(eps, 2)*g*k*(pow(chi, 2) -
1)*(pow(chi, 2) + 3)*(9*pow(chi, 2) -
13)*cos(3*k*x)*cosh(3*k*(h + y))/(omega*cosh(3*h*k)) +
a*g*k*cos(k*x)*cosh(k*(h + y))/(omega*cosh(h*k)) +
(3.0L/4.0L)*a*eps*g*k*pow(pow(chi, 2) - 1,
2)*cos(2*k*x)*cosh(2*k*(h + y))/(chi*omega*cosh(2*h*k));

vy = (3.0L/64.0L)*a*pow(eps, 2)*g*k*(pow(chi, 2) -
1)*(pow(chi, 2) + 3)*(9*pow(chi, 2) -
