...
class SynchronizerMPI
{
  ...
  map<Region, vector<BlockInfo> > region2infos;
      ...
      for (int d = 0; d < 3; ++d) {
        ...
        for (int s = 0; s < 2; ++s) {
          ...
          int end[3];
              ...
              (1 - s) * (blockstart[d] + thickness[d][s]) + s * blockend[d];
          ...
          for (int s = 0; s < 2; ++s) {
            {
              int neighbor_pe[3];
              neighbor_pe[dface] =
                  (mypeindex[dface] + 2 * s - 1 + pesize[dface]) %
                  pesize[dface];
                    ...
                    for (int b = 0; b < 2; ++b)
                      for (int a = 0; a < 2; ++a) {
...

                        {
                          const int sregion[3] = {
                              start[0] +
                                  (index[0] - neighbor[0]) * blocksize[0] -
                                  face_start[0],
                              start[1] +
                                  (index[1] - neighbor[1]) * blocksize[1] -
                                  face_start[1],
                              start[2] +
                                  (index[2] - neighbor[2]) * blocksize[2] -
                                  face_start[2]};
    ...
    Compressor* _newcompressor(
        Real* base, const size_t N, const bool compress) {
      if (N > 0) {
