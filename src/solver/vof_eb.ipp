...
template <class M_>
struct VofEmbed<M_>::Imp {
  using Owner = VofEmbed<M_>;
...
template <class M_>
void VofEmbed<M_>::SetPar(Par par) {
  imp->par = par;
}

template <class M_>
void VofEmbed<M_>::StartStep() {
  imp->StartStep();
}

template <class M_>
void VofEmbed<M_>::MakeIteration() {
  imp->MakeIteration();
}

template <class M_>
void VofEmbed<M_>::FinishStep() {
  imp->FinishStep();
}

template <class M_>
auto VofEmbed<M_>::GetField(Step l) const -> const FieldCell<Scal>& {
  return imp->fcu_.Get(l);
}

template <class M_>
auto VofEmbed<M_>::GetColor() const -> const FieldCell<Scal>& {
  return imp->fccl_;
}

template <class M_>
auto VofEmbed<M_>::GetImage(IdxCell c) const -> MIdx {
  return Trackerm<M>::Unpack(imp->fcim_[c]);
}

template <class M_>
auto VofEmbed<M_>::GetMask() const -> const FieldCell<bool>& {
  return imp->fci_;
}

template <class M_>
auto VofEmbed<M_>::GetAlpha() const -> const FieldCell<Scal>& {
  return imp->fca_;
}

template <class M_>
auto VofEmbed<M_>::GetNormal() const -> const FieldCell<Vect>& {
  return imp->fcn_;
}

template <class M_>
void VofEmbed<M_>::PostStep() {
  return imp->PostStep();
}

template <class M_>
void VofEmbed<M_>::DumpInterface(std::string fn) const {
  return imp->DumpInterface(fn);
}

template <class M_>
void VofEmbed<M_>::DumpInterfaceMarch(std::string fn) const {
