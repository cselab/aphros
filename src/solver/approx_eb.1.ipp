...
template <class T>
T Quad(Scal x, Scal um, Scal u, Scal up) {
  return (um * (x - 1) + up * (x + 1)) * x * 0.5 - u * (x - 1) * (x + 1);
}

>>>>>>> d29cf91e... geom: add notation (wip)
...
//                           //
  using Scal = typename M::Scal;
  auto& m = eb.GetMesh();
  auto dx = Notation::Direction(nf.abs().argmax());
  auto dy = Notation::Direction(1 - nf.abs().argmax()).sign(-nf);
  auto cw = c + dx;
  auto rw = m.GetCenter(cw);
  auto dist = (rx[dx] - rf[dx]) / nf[dx];
  auto yi = rf[dy] + dist * nf[dy];
  auto ui = Quad((y - rx[dy]) / h, fcu[cw - dy], fcu[cw], fcu[cw + dy]);
  return -(uf - ui) / dist;
>>>>>>> d29cf91e... geom: add notation (wip)
}
...
auto UEmbed<M>::InterpolateBilinearFaces(const FieldFace<T>& ffu, const EB& eb)
  ...
  for (auto f : eb.SuFaces()) {
    ...
    if (eb.IsRegular(f)) {
      ...
      auto r = (eb.GetFaceCenter(f) - m.GetCenter(f)).abs();
      ffb[f] = Bilinear(
          r[dx] / h, r[dy] / h, //
  ...
  return ffb;
      //auto f = (cm).face(dz);
      //auto fx = (cm + dx).face(dz);
      //auto fy = (cm + dy).face(dz);
      //auto fyx = (cm + dx + dy).face(dz);
}
