...
// Ref: J.R. Shewchuk - Lecture Notes on Geometric Robustness
//      http://www.cs.berkeley.edu/~jrs/meshpapers/robnotes.pdf
void triangleNormal(const Scal* a, const Scal* b, const Scal* c, Scal* ans) {
...
inline Scal vangle(const Scal* a, const Scal* b) {
...
void gramSchmidt(const Scal* x, const Scal* y, Scal* c) {
  Scal s;
...
bool contains(const Polygon* poly, const Scal* point) {
  Scal proj[3];
...
inline bool contains(const Hexahedron* hex, const Scal* p) {
  size_t i;
  Scal s;
...
inline std::pair<std::array<Scal, 2>, size_t> lineSphereIntersection(
    const Scal* origin, const Scal* direction, const Sphere* s) {
  std::array<Scal, 2> solutions = {
      {std::numeric_limits<Scal>::infinity(),
       std::numeric_limits<Scal>::infinity()}};
  Scal originRel[3];
...
// angle.
inline Scal regularizedWedge(Scal r, Scal d, Scal alpha) {
#ifndef NDEBUG
...
inline Scal regularizedWedge(Scal r, Scal d, Scal alpha, Scal z) {
  if (z >= Scal(0)) {
...
inline Scal generalWedge(
  ...
  if (dist < largeEpsilon) {
    ...
    cross[0] = p1.normal[0] * p0.normal[2] - p0.normal[0] * p1.normal[2];
    cross[1] = p0.normal[0] * p1.normal[1] - p1.normal[0] * p0.normal[1];
...
Scal generalWedge(
    ...
    const std::array<std::array<Scal[3], 2>, nrEdges>& intersections) {
...
Scal overlap(const Sphere& sOrig, const Hexahedron& elementOrig) {
  ...
  // is needed later on.
  std::array<std::array<Scal[3], 2>, nrEdges> eIntersections;
  ...
  for (size_t n = 0; n < nrEdges; ++n) {
    ...
    std::pair<std::array<Scal, 2>, size_t> solutions =
        lineSphereIntersection(start, direction, &s);
    ...
    // be avoided.
    if (!solutions.second || (solutions.first[0] >= Scal(1) - mediumEpsilon) ||
        solutions.first[1] <= mediumEpsilon ||
        (solutions.first[0] > Scal(0) && solutions.first[1] < Scal(1) &&
         (solutions.first[1] - solutions.first[0] < largeEpsilon))) {
      continue;
    } else {
      vMarked[edge_mapping[n][0][0]] = solutions.first[0] < Scal(0);

      vMarked[edge_mapping[n][0][1]] = solutions.first[1] > Scal(1);
    }
  ...
  for (size_t n = 0; n < nrVertices; ++n) {
    ...
    std::array<std::pair<size_t, Scal>, 3> distances;
    for (size_t i = 0; i < 3; ++i)
      distances[i] =
          std::make_pair(i, util_squared_norm(ip[i][0], ip[i][1], ip[i][2]));

    std::sort(
        distances.begin(), distances.end(),
        [](const std::pair<size_t, Scal>& a, const std::pair<size_t, Scal>& b)
    ...
    for (size_t e = 0; e < 3; ++e) {
      ...
      diff[0] = center[0] - s.center[0];
      diff[1] = center[1] - s.center[1];
      diff[2] = center[2] - s.center[2];
      Scal wedgeVolume =
