...
static int u(YoungParam param, double r, double th, double *vr,
...
static int v(YoungParam param, double r, double th, double *vr,
             double *vp, double *p, double *T)
{
    double rhov, rhou, muv, muu, hv, hu, gamma1, gamma0, T1, T0;
...
int young_set(YoungParam *p) {
    ...
    p->rhov = rhov; p->rhou = rhou; p->muv = muv; p->muu = muu;
    p->hv = hv; p->hu = hu; p->gamma1 = gamma1; p->gamma0 = gamma0;
    p->T1 = T1; p->T0 = T0; p->R = R;
    return 0;
}
...
int young_fields(double y, double z, /**/ double *pvy, double *pvz, double *p, double *T) {
    double R, r, th;
    double vr, vth, vy, vz;
    ...
    if (r > R) v(param, r, th, &vr, &vth, p, T);
    else       u(param, r, th, &vr, &vth, p, T);
