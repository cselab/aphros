...
class BlockLabMPI : public MyBlockLab {
 ...
 public:
  ...
  void load(const BlockInfo& info) {
    ...
    const bool xskin =
        (info.index[0] == xorigin || info.index[0] == xorigin + mybpd[0] - 1);
    const bool yskin =
        (info.index[1] == yorigin || info.index[1] == yorigin + mybpd[1] - 1);
    const bool zskin =
        (info.index[2] == zorigin || info.index[2] == zorigin + mybpd[2] - 1);
         ...
         zskin)) // && (!any_boundary || any_boundary && any_periodic))
    {
      ...
      const int rsx =
          (!xperiodic && info.index[0] == 0) ? 0 : this->m_stencilStart[0];
      const int rex = (!xperiodic && info.index[0] == gLastX)
                          ? BlockType::sizeX
                          : (BlockType::sizeX + this->m_stencilEnd[0] - 1);
      const int rsy =
          (!yperiodic && info.index[1] == 0) ? 0 : this->m_stencilStart[1];
      const int rey = (!yperiodic && info.index[1] == gLastY)
                          ? BlockType::sizeY
                          : (BlockType::sizeY + this->m_stencilEnd[1] - 1);
      const int rsz =
          (!zperiodic && info.index[2] == 0) ? 0 : this->m_stencilStart[2];
      const int rez = (!zperiodic && info.index[2] == gLastZ)
                          ? BlockType::sizeZ
      ...
      refSynchronizerMPI->fetch(
          (const Real*)info.ptrBlock, dst, this->m_stencilStart[0],
          this->m_stencilStart[1], this->m_stencilStart[2],
          this->m_cacheBlock->getSize()[0], this->m_cacheBlock->getSize()[1],
          this->m_cacheBlock->getSize()[2], sizeof(ET) / sizeof(Real), rsx, rex,
          rsy, rey, rsz, rez);
    }
  }
