...
class SynchronizerMPI {
  ...
  map<Real*, vector<SubpackInfo> > _setup(
    ...
    for (int d = 0; d < 3; ++d) {
      ...
      for (int s = 0; s < 2; ++s) {
        const int NFACEBLOCK = NC * thickness[d][s] * blocksize[dim_other1] *
        ...
        for (int b = 0; b < n2; ++b)
          for (int a = 0; a < n1; ++a) {
            ...
            PackInfo info = {(Real*)globalinfos[blockid].ptrBlock,
    ...
    for (int d = 0; d < 3; ++d) {
      ...
      for (int b = 0; b < 2; ++b)
        for (int a = 0; a < 2; ++a) {
          const int NEDGEBLOCK = NC * blocksize[d] * thickness[dim_other2][b] *
    ...
    if (computesubregions) {
      for (int dface = 0; dface < 3; ++dface) {
        ...
        for (int s = 0; s < 2; ++s) {
          ...
          const int NFACEBLOCK = NC * thickness[dface][s] *
              ...
              for (int dedge = 0; dedge < 3; ++dedge) // iterate over edges
              {
                ...
                for (int b = 0; b < 2; ++b)
                  for (int a = 0; a < 2; ++a) {
                    ...
                    if (c2i.find(
                            I3(origin[0] + neighbor[0], origin[1] + neighbor[1],
...

                    {
                      ...
                      // neighbor[1], neighbor[2]); printf("face: %d %d\n",
                      // dface, s);
                      // printf("edge: %d %d %d\n", dedge, a, b);
                      ...
                      SubpackInfo subinfo = {ptrBlock,   src_base,   start[0],
                                             start[1],   start[2],   end[0],
                                             end[1],     end[2],     sregion[0],
                                             sregion[1], sregion[2], L[0],
                                             L[1]};
              ...
              for (int z = 0; z < 2; ++z)
                for (int y = 0; y < 2; ++y)
                  for (int x = 0; x < 2; ++x) {
                    ...
                    const int start[3] = {x * (blockend[0] - thickness[0][1]) +
                                              (1 - x) * blockstart[0],
      ...
      for (int d = 0; d < 3; ++d) {
        ...
        for (int b = 0; b < 2; ++b)
          for (int a = 0; a < 2; ++a) {
            ...
            const int n = bpd[d];
            const int NEDGEBLOCK = NC * blocksize[d] *
                ...
                for (int y = 0; y < 2; ++y)
                  for (int x = 0; x < 2; ++x) {
...

                    {
                      ...
                      // n1*neighbor[dim_other2face]);
                      Real* src_base =
                          data.edges[d][b][a] + NEDGEBLOCK * neighbor[d];
    ...
    for (int z = 0; z < 2; ++z)
      for (int y = 0; y < 2; ++y)
        for (int x = 0; x < 2; ++x) {
          const int NCORNERBLOCK =
              NC * thickness[0][x] * thickness[1][y] * thickness[2][z];
 ...
 public:
  SynchronizerMPI(
      const int synchID, StencilInfo stencil, vector<BlockInfo> globalinfos,
      MPI_Comm cartcomm, const int mybpd[3], const int blocksize[3])
      : synchID(synchID)
      , stencil(stencil)
    ...
    const int origin[3] = {mypeindex[0] * mybpd[0], mypeindex[1] * mybpd[1],
                           mypeindex[2] * mybpd[2]};
...

    {
      ...
      recv_subpackinfos = _setup<true>(
          recv, recv_thickness, blockstart, blockend, origin, packinfos);
  ...
  virtual void sync(
      size_t gptfloats, MPI_Datatype MPIREAL, const int timestamp) {
    ...
    // 1. pack
    {
      ...
      if (!contiguous) {
        for (int i = 0; i < N; ++i) {
          ...
          pack(
              info.block, info.pack, gptfloats, &selcomponents.front(), NC,
              info.sx, info.sy, info.sz, info.ex, info.ey, info.ez, bx, by);
        }
      } else {
        ...
        for (int i = 0; i < N; ++i) {
          PackInfo info = send_packinfos[i];
          pack_stripes(
              info.block, info.pack, gptfloats, selstart, selend, info.sx,
              info.sy, info.sz, info.ex, info.ey, info.ez, bx, by);
        }
      }
    }
      ...
      for (int d = 0; d < 3; ++d) {
        ...
        for (int s = 0; s < 2; ++s) {
          ...
          if (NFACE_SEND > 0) {
            MPI_Request req;
            MPI_Isend(
                send.faces[d][s], NFACE_SEND, MPIREAL, _rank(neighbor_index),
          ...
          if (NFACE_RECV > 0) {
            MPI_Request rc;
            MPI_Irecv(
                recv.faces[d][s], NFACE_RECV, MPIREAL, _rank(neighbor_index),
          ...
          for (int b = 0; b < 2; ++b)
            for (int a = 0; a < 2; ++a) {
              ...
              if (NEDGE_RECV > 0) {
                MPI_Request rc;
                MPI_Irecv(
                    recv.edges[d][b][a], NEDGE_RECV, MPIREAL,
                    _rank(neighbor_index), 12 * timestamp + 4 * d + 2 * b + a,
                    cartcomm, &rc);
              ...
              if (NEDGE_SEND > 0) {
                MPI_Request req;
                MPI_Isend(
                    send.edges[d][b][a], NEDGE_SEND, MPIREAL,
            ...
            for (int y = 0; y < 2; ++y)
              for (int x = 0; x < 2; ++x) {
                ...
                if (NCORNERBLOCK_RECV) {
                  MPI_Request rc;
                  MPI_Irecv(
                      recv.corners[z][y][x], NCORNERBLOCK_RECV, MPIREAL,
                      _rank(neighbor_index), 8 * timestamp + 4 * z + 2 * y + x,
                      cartcomm, &rc);
                ...
                if (NCORNERBLOCK_SEND) {
                  MPI_Request req;
                  MPI_Isend(
                      send.corners[z][y][x], NCORNERBLOCK_SEND, MPIREAL,
                      _rank(neighbor_index),
