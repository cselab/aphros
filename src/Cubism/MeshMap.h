...
class MeshDensity {
 public:
  ...
  virtual void compute_spacing(
      const double xS, const double xE, const size_t ncells, double* const ary,
      const size_t ghostS = 0, const size_t ghostE = 0,
      double* const ghost_spacing = NULL) const = 0;
};
...
class UniformDensity : public MeshDensity {
 public:
  ...
  virtual void compute_spacing(
      const double xS, const double xE, const size_t ncells, double* const ary,
      const size_t ghostS = 0, const size_t ghostE = 0,
      double* const ghost_spacing = NULL) const {
...
class MeshMap {
 public:
  MeshMap(
      const double xS, const double xE, const size_t Nblocks, const size_t bs)
      : m_xS(xS)
      , m_xE(xE)
      , m_extent(xE - xS)
      , m_Nblocks(Nblocks)
  ...
  void init(
      const MeshDensity* const kernel, const size_t ghostS = 0,
      const size_t ghostE = 0, double* const ghost_spacing = NULL) {
    _alloc();

    kernel->compute_spacing(
        m_xS, m_xE, m_Ncells, m_grid_spacing, ghostS, ghostE, ghost_spacing);
  ...
  inline double* data_grid_spacing() {
