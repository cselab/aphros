 ...
 public:
  GridMPI(
    ...
    const double blockwidth =
        (maxextent / std::max(nX * npeX, std::max(nY * npeY, nZ * npeZ)));
    const double extents[3] = {blockwidth * nX * npeX, blockwidth * nY * npeY,
                               blockwidth * nZ * npeZ};
    const size_t nBlocks[3] = {(size_t)nX * npeX, (size_t)nY * npeY,
                               (size_t)nZ * npeZ};
    for (int i = 0; i < 3; ++i) {
      MeshMap<Block>* m =
          new MeshMap<Block>(0.0, extents[i], nBlocks[i], blocksize[i]);
    ...
    const double h_gridpoint =
        maxextent / (double)max(
    ...
    for (size_t i = 0; i < vInfo.size(); ++i) {
      ...
      info.h = info.h_gridpoint *
               blocksize[0]; // only for blocksize[0]=blocksize[1]=blocksize[2]
      ...
      for (int j = 0; j < 3; ++j) {
        ...
        info.ptr_grid_spacing[j] =
            this->m_mesh_maps[j]->get_grid_spacing(info.index[j]);
      }
  ...
  GridMPI(
      MeshMap<Block>* const mapX, MeshMap<Block>* const mapY,
      MeshMap<Block>* const mapZ, const int npeX, const int npeY,
      const int npeZ, const int nX = 0, const int nY = 0, const int nZ = 0,
      const MPI_Comm comm = MPI_COMM_WORLD)
  ...
  Synch& sync(Processing& p) {
    ...
    if (itSynchronizerMPI == SynchronizerMPIs.end()) {
      queryresult = new Synch(
          SynchronizerMPIs.size(), stencil, getBlocksInfo(), cartcomm, mybpd,
          blocksize);
