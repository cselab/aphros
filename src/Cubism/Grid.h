...
#pragma omp for schedule(static)
 ...
 public:
  ...
  Grid(
      const size_t NX, const size_t NY = 1, const size_t NZ = 1,
    ...
    for (int i = 0; i < 3; ++i) {
      MeshMap<Block>* m =
          new MeshMap<Block>(0.0, extents[i], nBlocks[i], bs[i]);
    ...
    for (size_t iz = 0; iz < NZ; iz++)
      for (size_t iy = 0; iy < NY; iy++)
        for (size_t ix = 0; ix < NX; ix++) {
          ...
          m_vInfo.push_back(
              BlockInfo(blockID, idx, origin, h, hc, _linaccess(blockID)));
        }
  }

  Grid(
      MeshMap<Block>* const mapX, MeshMap<Block>* const mapY,
      MeshMap<Block>* const mapZ, const int _NX, const int _NY = 1,
      const int _NZ = 1)
      : m_blocks(NULL)
      , NX(_NX)
      , NY(_NY)
    ...
    for (size_t iz = 0; iz < NZ; iz++)
      for (size_t iy = 0; iy < NY; iy++)
        for (size_t ix = 0; ix < NX; ix++) {
          ...
          m_vInfo.push_back(
              BlockInfo(blockID, idx, mapX, mapY, mapZ, _linaccess(blockID)));
        }
  }
  ...
  void setup(const size_t nX, const size_t nY, const size_t nZ) {
    ...
    // dead code (?)
    std::cout << "Setting up the grid with " << nX << "x" << nY << "x" << nZ
              << " blocks ...";
  ...
  virtual bool avail(int, int, int) const {
    return true;
  }
...
std::ostream& operator<<(
    std::ostream& out, const Grid<Block, allocator>& grid) {
  // save metadata
  out << grid.getBlocksPerDimension(0) << " " << grid.getBlocksPerDimension(1)
