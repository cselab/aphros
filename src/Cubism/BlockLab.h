 ...
 */
template <
    typename TBlock, template <typename X> class allocator = std::allocator,
...
class BlockLab {
 ...
 public:
  ...
  inline ElementType* getBuffer() const {
    return &m_cacheBlock->LinAccess(0);
  }

  void prepare(
      Grid<BlockType, allocator>& grid, int startX, int endX, int startY,
      int endY, int startZ, int endZ, const bool istensorial) {
  ...
  void prepare(
      Grid<BlockType, allocator>& grid, const int stencil_start[3],
    ...
    if (m_cacheBlock == NULL ||
      ...
      m_cacheBlock =
          allocator<Matrix3D<ElementType, true, allocator> >().allocate(1);

      allocator<Matrix3D<ElementType, true, allocator> >().construct(
          m_cacheBlock, Matrix3D<ElementType, true, allocator>());
    ...
    // m_stencilStart[1], m_stencilStart[2], 			m_stencilEnd[0],
    // m_stencilEnd[1], m_stencilEnd[2]);
  }
  ...
  void load(const BlockInfo& info) {
    ...
    // 1.
    {
      ...
      for (int iz = _iz0; iz < _iz1; iz++) {
        ...
        for (int iy = _iy0; iy < _iy1; iy += 4) {
          ElementType* ptrDestination0 =
              &m_cacheBlock->LinAccess(my_izx + (iy + 0) * m_vSize0);
...
#endif
        {
          ElementType* ptrDestination =
              &m_cacheBlock->Access(0 - m_stencilStart[0], iy, iz);

          // for(int ix=0; ix<nX; ix++, ptrSource++, ptrDestination++)
          //	*ptrDestination = (ElementType)*ptrSource;
          memcpy2((char*)ptrDestination, (char*)ptrSource, nbytes);
          // for (int ix = 0; ix < nX; ix++)  ptrDestination[ix] =
          // ptrSource[ix];
    ...
    // 2.
    {
      ...
      const bool xskin =
          info.index[0] == 0 ||
          info.index[0] == (int)grid.getBlocksPerDimension(0) - 1;
      const bool yskin =
          info.index[1] == 0 ||
          info.index[1] == (int)grid.getBlocksPerDimension(1) - 1;
      const bool zskin =
          info.index[2] == 0 ||
          info.index[2] == (int)grid.getBlocksPerDimension(2) - 1;
      ...
      for (int icode = 0; icode < 27; icode++) {
        ...
        if (!grid.avail(
                info.index[0] + code[0], info.index[1] + code[1],
                info.index[2] + code[2]))
          continue;
      ...
      // iy-m_stencilStart[1], iz-m_stencilStart[2]);
          ...
          if ((e[1] - s[1]) % 4 != 0) {
            for (int iy = s[1]; iy < e[1]; iy++) {
              char* ptrDest = (char*)&m_cacheBlock->LinAccess(
...
#endif
    return m_cacheBlock->Access(
        ix - m_stencilStart[0], iy - m_stencilStart[1], iz - m_stencilStart[2]);
  }
...
template <
    typename BlockType, template <typename X> class allocator = std::allocator,
    typename ElementTypeT = typename BlockType::ElementType>
class EmptyBlockLab : BlockLab<BlockType, allocator, ElementTypeT> {
 ...
 public:
  ...
  inline void prepare(
    ...
    if (current_size[0] != BlockType::sizeX + this->m_stencilEnd[0] -
                               this->m_stencilStart[0] - 1 ||
        current_size[1] != BlockType::sizeY + this->m_stencilEnd[1] -
                               this->m_stencilStart[1] - 1 ||
        current_size[2] != BlockType::sizeZ + this->m_stencilEnd[2] -
                               this->m_stencilStart[2] - 1) {
      current_size[0] = BlockType::sizeX + this->m_stencilEnd[0] -
                        this->m_stencilStart[0] - 1;
      current_size[1] = BlockType::sizeY + this->m_stencilEnd[1] -
                        this->m_stencilStart[1] - 1;
      current_size[2] = BlockType::sizeZ + this->m_stencilEnd[2] -
    ...
    this->m_state =
        BlockLab<BlockType, allocator, ElementTypeT>::eMRAGBlockLab_Prepared;
  }
};
