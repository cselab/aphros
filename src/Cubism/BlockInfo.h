...
struct BlockInfo {
  ...
  BlockInfo(
      long long ID, const int idx[3], MeshMap<TBlock>* const mapX,
      MeshMap<TBlock>* const mapY, MeshMap<TBlock>* const mapZ,
      void* ptr = NULL, const bool _special = false)
      : blockID(ID), ptrBlock(ptr), special(_special) {
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      if (bUniform[i])
        uniform_grid_spacing[i] = h[0];
      else
        uniform_grid_spacing[i] = -1.0;
    }
  }
