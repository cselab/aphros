...
static gdouble transverse_term(
    FttCell* cell, gdouble* msize, FttComponent c,
...
static gdouble interpolate_1D1(
    const FttCell* cell, FttDirection dright, FttDirection dup, gdouble x) {
  FttCell* n;
  FttDirection dleft;
  GfsStateVector* s;
...
static gdouble interpolate_2D1(
    const FttCell* cell, FttDirection dright, FttDirection d1, FttDirection d2,
    gdouble x, gdouble y) {
  FttCell *n1, *n2;
  gdouble x1 = 0., y1 = 1.;
  gdouble x2 = 1., y2 = 0.;
...
gdouble gfs_face_upwinded_value(
  ...
  if (GFS_FACE_FRACTION(face) == 0.) return 0.;
  ...
  switch (ftt_face_type(face)) {
    ...
    case FTT_FINE_COARSE:
      if (un > 0.)
        ...
        vcoarse = interpolate_2D1(
            face->neighbor, face->d, dp[0], dp[1], 1. / 4., 1. / 4.);
#endif /* FTT_3D */
        if (un == 0.)
...
void gfs_face_advection_flux(
  ...
  if (!FTT_FACE_DIRECT(face)) flux = -flux;
...
void gfs_face_advected_normal_velocity(
  ...
  if (GFS_FACE_FRACTION_RIGHT(face) == 0.) return;
...
static void set_merged(FttCell* cell) {
  ...
  if (!gfs_cell_is_small(cell))
    ...
    if (abest == 0.)
      g_warning(
          "file %s: line %d (%s): cannot merge small cell: %g", __FILE__,
          __LINE__, G_GNUC_PRETTY_FUNCTION, solid->a);
  }
}
...
void gfs_set_merged(GfsDomain* domain) {
  ...
  gfs_domain_traverse_mixed(
      domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
...
static void add_merged(GSList** merged, FttCell* cell) {
  if ((cell->flags & GFS_FLAG_USED) == 0) {
    ...
    *merged = g_slist_prepend(*merged, cell);
    ...
    if (solid && solid->merged) add_merged(merged, solid->merged);
...
void gfs_advection_params_write(GfsAdvectionParams* par, FILE* fp) {
  ...
  if (par->moving_order != 1) fputs("  moving_order = 2\n", fp);
  if (par->sink[0]) {
    fputs("  vx = ", fp);
...
void gfs_advection_params_read(GfsAdvectionParams* par, GtsFile* fp) {
  ...
  if (gradient) {
    if (!strcmp(gradient, "gfs_center_gradient"))
      ...
      gts_file_variable_error(
          fp, var, "gradient", "unknown gradient parameter `%s'", gradient);
    g_free(gradient);
  }
  ...
  if (flux) {
    if (!strcmp(flux, "gfs_face_advection_flux"))
      ...
      gts_file_variable_error(
          fp, var, "flux", "unknown flux parameter `%s'", flux);
    g_free(flux);
  }
  ...
  if (scheme) {
    if (!strcmp(scheme, "godunov"))
      ...
      gts_file_variable_error(
          fp, var, "scheme", "unknown scheme parameter `%s'", scheme);
    g_free(scheme);
  }
}
