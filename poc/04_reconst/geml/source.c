...
gdouble gfs_variable_mac_source(GfsVariable* v, FttCell* cell) {
  ...
  while (i) {
    ...
    if (s->mac_value) sum += (*s->mac_value)(s, cell, v);
    i = i->next;
  }
  return sum;
}
...
static void add_sources(FttCell* cell, SourcePar* p) {
  ...
  while (i) {
    ...
    if (s->centered_value) sum += (*s->centered_value)(s, cell, p->v);
    i = i->next;
  }
 ...
 */
void gfs_domain_variable_centered_sources(
    GfsDomain* domain, GfsVariable* v, GfsVariable* sv, gdouble dt) {
 ...
 */
GfsVariable* gfs_domain_variable_fluxes(
    GfsDomain* domain, GfsVariable* v, gdouble dt) {
  GfsVariable* sv = NULL;

  g_return_val_if_fail(domain != NULL, NULL);
  g_return_val_if_fail(v != NULL, NULL);

  if (!v->sources) return NULL;
    ...
    if (GFS_SOURCE_GENERIC(i->data)->flux) {
      if (sv == NULL) {
        ...
        gfs_domain_traverse_leaves(
            domain, (FttCellTraverseFunc)gfs_cell_reset, sv);
      }
...
GfsSourceGenericClass* gfs_source_generic_class(void) {
  ...
  if (klass == NULL) {
    ...
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_event_class()), &gfs_source_generic_info);
  }
 ...
 * none was found.
 */
GfsSourceGeneric* gfs_source_find(
    GfsVariable* v, GfsSourceGenericClass* klass) {
  g_return_val_if_fail(v != NULL, NULL);
  ...
  if (v->sources) {
    ...
    while (i) {
      ...
      if (gts_object_is_from_class(o, klass)) return GFS_SOURCE_GENERIC(o);
      i = i->next;
    }
  }
  return NULL;
...
static void source_scalar_read(GtsObject** o, GtsFile* fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_source_scalar_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_source_scalar_class())->parent_class->read)(o, fp);
  ...
  if (fp->type == '(') {
    /* vector */
    if (!gfs_read_variable_vector(fp, source->vector, domain)) return;
    gint i;
...
GfsSourceGenericClass* gfs_source_scalar_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo gfs_source_scalar_info = {
        ...
        (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_source_generic_class()), &gfs_source_scalar_info);
  }
...
GfsSourceGenericClass* gfs_source_velocity_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo source_info = {
        ...
        (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_source_generic_class()), &source_info);
  }
...
static void source_destroy(GtsObject* o) {
  ...
  if (s->intensity) gts_object_destroy(GTS_OBJECT(s->intensity));

  gint i;
  for (i = 0; i < FTT_DIMENSION; i++)
    if (s->intensity_v[i]) gts_object_destroy(GTS_OBJECT(s->intensity_v[i]));

  (*GTS_OBJECT_CLASS(gfs_source_class())->parent_class->destroy)(o);
}

static gdouble source_face_value(
    GfsSourceGeneric* s, FttCellFace* face, GfsVariable* v) {
  return gfs_function_face_value(GFS_SOURCE(s)->intensity, face);
}

static gdouble source_vector_face_value(
    GfsSourceGeneric* s, FttCellFace* face, GfsVariable* v) {
  FttVector p, u;
  int i;
  for (i = 0; i < FTT_DIMENSION; i++)
...
static gdouble source_value(
    GfsSourceGeneric* s, FttCell* cell, GfsVariable* v) {
  return gfs_function_value(GFS_SOURCE(s)->intensity, cell);
}

static gdouble source_vector_value(
    GfsSourceGeneric* s, FttCell* cell, GfsVariable* v) {
...
static void source_read(GtsObject** o, GtsFile* fp) {
  if (GTS_OBJECT_CLASS(gfs_source_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_source_class())->parent_class->read)(o, fp);
  if (fp->type == GTS_ERROR) return;
  ...
  if (s->v) {
    ...
    /* vector */
    if (!gfs_read_function_vector(
            fp, s->vector, GFS_SOURCE(s)->intensity_v,
...
static void source_write(GtsObject* o, FILE* fp) {
  if (GTS_OBJECT_CLASS(gfs_source_class())->parent_class->write)
    (*GTS_OBJECT_CLASS(gfs_source_class())->parent_class->write)(o, fp);
  GfsSource* s = GFS_SOURCE(o);
  if (s->intensity)
...
GfsSourceGenericClass* gfs_source_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo source_info = {"GfsSource",
                                      sizeof(GfsSource),
                                      sizeof(GfsSourceGenericClass),
                                      ...
                                      (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_source_scalar_class()), &source_info);
  }
...
static void source_control_read(GtsObject** o, GtsFile* fp) {
  ...
  GFS_SOURCE_CONTROL(*o)->intensity =
      gfs_function_new(gfs_function_class(), 0.);
  gfs_function_set_units(
      GFS_SOURCE_CONTROL(*o)->intensity, GFS_SOURCE_SCALAR(*o)->v->units);
  gfs_function_read(
      GFS_SOURCE_CONTROL(*o)->intensity, gfs_object_simulation(*o), fp);
}
...
static gboolean source_control_event(GfsEvent* event, GfsSimulation* sim) {
  if ((*gfs_event_class()->event)(event, sim)) {
    ...
    s->s = sim->advection_params.dt > 0. && su.sv > 0.
               ? (gfs_function_value(s->intensity, NULL) - su.s / su.sv) /
...
static gdouble source_control_value(
    GfsSourceGeneric* s, FttCell* cell, GfsVariable* v) {
...
GfsSourceGenericClass* gfs_source_control_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo source_control_info = {
        ...
        (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_source_scalar_class()), &source_control_info);
  }
...
static void source_control_field_destroy(GtsObject* o) {
  ...
  (*GTS_OBJECT_CLASS(gfs_source_control_field_class())->parent_class->destroy)(
      o);
}

static gdouble source_control_field_face_value(
    GfsSourceGeneric* s, FttCellFace* face, GfsVariable* v) {
  return gfs_face_interpolated_value_generic(
      face, GFS_SOURCE_CONTROL_FIELD(s)->s);
}

static void source_control_field_read(GtsObject** o, GtsFile* fp) {
  (*GTS_OBJECT_CLASS(gfs_source_control_field_class())->parent_class->read)(
      o, fp);
  if (fp->type == GTS_ERROR) return;
...
static void source_control_field_write(GtsObject* o, FILE* fp) {
  (*GTS_OBJECT_CLASS(gfs_source_control_field_class())->parent_class->write)(
      o, fp);
  fprintf(
      fp, " %d %g", GFS_SOURCE_CONTROL_FIELD(o)->level,
      GFS_SOURCE_CONTROL_FIELD(o)->tau);
}

static void source_control_field_root(FttCell* root, GfsSourceControlField* f) {
  Sum su = {GFS_SOURCE_SCALAR(f)->v, 0., 0.};
  ftt_cell_traverse(
      root, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1, (FttCellTraverseFunc)sum,
      &su);
  GFS_VALUE(root, f->s) =
      su.sv > 0. ? (gfs_function_value(GFS_SOURCE_CONTROL(f)->intensity, root) -
                    su.s / su.sv) /
                       f->tau
                 : 0.;
}
...
static void extrapolate(FttCell* cell, ExtraData* p) {
  ...
  GFS_VALUE(cell, p->v) =
      gfs_interpolate_from_corners(p->root, pos, p->corners);
}
...
static gboolean source_control_field_event(
    GfsEvent* event, GfsSimulation* sim) {
  if ((*gfs_event_class()->event)(event, sim)) {
    GfsDomain* domain = GFS_DOMAIN(sim);
    ...
    gfs_domain_cell_traverse(
        ...
        f->level, (FttCellTraverseFunc)source_control_field_root, event);
    gfs_restore_fpe_for_function(GFS_SOURCE_CONTROL(event)->intensity);
    gfs_domain_bc(
        domain, FTT_TRAVERSE_LEVEL | FTT_TRAVERSE_LEAFS, f->level, f->s);
    gfs_domain_cell_traverse(
        domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEVEL | FTT_TRAVERSE_NON_LEAFS,
        f->level, (FttCellTraverseFunc)extrapolate_field, event);
...
static gdouble source_control_field_value(
    GfsSourceGeneric* s, FttCell* cell, GfsVariable* v) {
...
GfsSourceGenericClass* gfs_source_control_field_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo source_control_field_info = {
        ...
        (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_source_control_class()),
...
static void source_flux_read(GtsObject** o, GtsFile* fp) {
  ...
  gfs_function_set_units(
      f->intensity, GFS_SOURCE_SCALAR(f)->v->units + FTT_DIMENSION);
...
static gdouble source_flux_value(
    GfsSourceGeneric* s, FttCell* cell, GfsVariable* v) {
  return GFS_SOURCE_FLUX(s)->s *
         gfs_function_value(GFS_SOURCE_FLUX(s)->fraction, cell);
}
...
GfsSourceGenericClass* gfs_source_flux_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo source_flux_info = {
        ...
        (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_source_scalar_class()), &source_flux_info);
  }
...
static gboolean diffusion_event(GfsEvent* event, GfsSimulation* sim) {
  ...
  if (!gfs_function_is_constant(d->val)) {
    ...
    if (d->mu != gfs_function_get_variable(d->val)) {
      gfs_catch_floating_point_exceptions();
      gfs_domain_cell_traverse(
          GFS_DOMAIN(sim), FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
          (FttCellTraverseFunc)update_mu, event);
      gfs_restore_fpe_for_function(d->val);
    }
    gfs_domain_cell_traverse(
        GFS_DOMAIN(sim), FTT_POST_ORDER, FTT_TRAVERSE_NON_LEAFS, -1,
...
static gdouble diffusion_concentration_face(
    GfsDiffusion* d, FttCellFace* face) {
  return diffusion_face(d, face) *
         gfs_vof_face_fraction(face, GFS_VARIABLE_TRACER_VOF(d->phase));
}
...
GfsDiffusionClass* gfs_diffusion_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo diffusion_info = {
        ...
        (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_event_class()), &diffusion_info);
  }
...
static GfsSourceDiffusion* previous_diffusion_source(
    GfsVariable* v, GfsSourceDiffusion* d) {
  GSList* i;
  i = GTS_SLIST_CONTAINER(v->sources)->items;
  while (i) {
...
static void source_diffusion_read(GtsObject** o, GtsFile* fp) {
  ...
  if (fp->type != GTS_STRING)
    ...
    gts_file_next_token(fp);
    gfs_function_set_units(
        d->D->val, 2. + GFS_SOURCE_SCALAR(*o)->v->units - d->phi->units);
  }
  if (GFS_IS_VARIABLE_VOF_CONCENTRATION(GFS_SOURCE_SCALAR(d)->v)) {
    d->D->phase = GFS_VARIABLE(
        GFS_VARIABLE_VOF_CONCENTRATION(GFS_SOURCE_SCALAR(d)->v)->vof);
  ...
  if (GFS_SOURCE_SCALAR(d)->v->component < FTT_DIMENSION &&
      !gfs_function_is_constant(d->D->val))
    g_warning(
...
static void source_diffusion_write(GtsObject* o, FILE* fp) {
  ...
  if (d->phi != GFS_SOURCE_SCALAR(o)->v) fprintf(fp, " %s", d->phi->name);
}
...
static gdouble source_diffusion_value(
  ...
  GfsFunction* alpha = phi->component < FTT_DIMENSION
                           ? gfs_object_simulation(s)->physical_params.alpha
                           : NULL;
  return (alpha ? gfs_function_value(alpha, cell) : 1.) * (g.b - g.a * v0) /
         (h * h);
}
...
GfsSourceGenericClass* gfs_source_diffusion_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo source_diffusion_info = {
        ...
        (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_source_scalar_class()), &source_diffusion_info);
  }
...
static void source_diffusion_explicit_flux(
    GfsSourceGeneric* s, GfsDomain* domain, GfsVariable* v, GfsVariable* sv,
    gdouble dt) {
  GfsVariable* phi = GFS_SOURCE_DIFFUSION(s)->phi;
  gfs_domain_surface_bc(domain, phi);
  gfs_diffusion_coefficients(
      domain, GFS_SOURCE_DIFFUSION(s), dt, NULL, NULL, NULL, 1.);
  FluxPar p = {s, phi, sv};
  gfs_domain_traverse_leaves(
      domain, (FttCellTraverseFunc)add_diffusion_explicit_flux, &p);
}

static void gfs_source_diffusion_explicit_read(GtsObject** o, GtsFile* fp) {
  if (GTS_OBJECT_CLASS(gfs_source_diffusion_explicit_class())
          ->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_source_diffusion_explicit_class())
...
static void cell_diffusion_stability(FttCell* cell, StabilityParams* par) {
  ...
  if (Dmax > 0.) {
    gdouble dtmax =
        h * h /
        (Dmax * (par->alpha ? gfs_function_value(par->alpha, cell) : 1.))
        /* fixme: why 10? */
        / 10.;
    if (dtmax < par->dtmax) par->dtmax = dtmax;
  }
}

static gdouble source_diffusion_stability(
    GfsSourceGeneric* s, GfsSimulation* sim) {
  StabilityParams par;
...
static void gfs_source_diffusion_explicit_class_init(
...
GfsSourceGenericClass* gfs_source_diffusion_explicit_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo gfs_source_diffusion_explicit_info = {
        ...
        (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_source_diffusion_class()),
...
static void compute_transverse(FttCell* cell, FluxPar* p) {
  ...
  GFS_VALUE(cell, p->tv) =
      gfs_center_gradient(
          cell, p->v->component, v[(p->v->component + 1) % FTT_DIMENSION]->i) /
      h;
}
...
static gdouble source_viscosity_non_diffusion_value(
    GfsSourceGeneric* s, FttCell* cell, GfsVariable* v) {
  gdouble src = 0.;
  ...
  if (mu != NULL) {
    ...
    for (j = 0; j < FTT_DIMENSION; j++)
      a +=
          (gfs_center_gradient(cell, c, u[j]->i) *
  ...
  if (v->domain->viscous_metric_explicit)
    src += (*v->domain->viscous_metric_explicit)(
        v->domain, cell, v, GFS_SOURCE_DIFFUSION(s)->D);

  return src;
}

static gdouble source_viscosity_value(
    GfsSourceGeneric* s, FttCell* cell, GfsVariable* v) {
  return (
      source_diffusion_value(s, cell, v) +
...
GfsSourceGenericClass* gfs_source_viscosity_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo source_viscosity_info = {
        ...
        (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_source_diffusion_class()), &source_viscosity_info);
  }
...
static gdouble source_viscosity_stability(
    GfsSourceGeneric* s, GfsSimulation* sim) {
  StabilityParams par;
...
static void add_axisymmetric_term(FttCell* cell, FluxPar* p) {
  ...
  GFS_VALUE(cell, p->sv) -=
      (alpha ? gfs_function_value(alpha, cell) : 1.) * 2. *
      gfs_source_diffusion_cell(GFS_SOURCE_DIFFUSION(p->s), cell) *
      GFS_VALUE(cell, p->v) * a * a /
      gfs_domain_cell_fraction(p->v->domain, cell) * p->dt;
}

static void source_viscosity_explicit_flux(
    GfsSourceGeneric* s, GfsDomain* domain, GfsVariable* v, GfsVariable* sv,
    gdouble dt) {
  FluxPar p;
  ...
  gfs_traverse_and_bc(
      domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
      (FttCellTraverseFunc)compute_transverse, &p, p.tv, p.tv);
  gfs_domain_traverse_leaves(
      domain, (FttCellTraverseFunc)add_viscosity_explicit_flux, &p);
  if (GFS_IS_AXI(domain) && v->component == FTT_Y)
    gfs_domain_traverse_leaves(
        domain, (FttCellTraverseFunc)add_axisymmetric_term, &p);
...
GfsSourceGenericClass* gfs_source_viscosity_explicit_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo source_viscosity_explicit_info = {
        ...
        (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_source_viscosity_class()),
...
static void source_coriolis_destroy(GtsObject* o) {
  ...
  if (s->omegaz) gts_object_destroy(GTS_OBJECT(s->omegaz));
  if (s->drag) gts_object_destroy(GTS_OBJECT(s->drag));

  for (c = 0; c < FTT_DIMENSION; c++)
    if (s->u[c]) gts_object_destroy(GTS_OBJECT(s->u[c]));
    ...
    if (v->sources) {
      ...
      while (i) {
        if (i->data != *o && GFS_IS_SOURCE_CORIOLIS(i->data)) {
          gts_file_error(
              fp, "variable '%s' cannot have multiple Coriolis source terms",
              v->name);
          return;
        }
        i = i->next;
...
#if !FTT_2D
  if (fp->type == '{') {
    GtsFileVariable var[] = {{GTS_DOUBLE, "x", TRUE, &s->d[0]},
    ...
    if (fp->type == GTS_ERROR) return;
    gdouble norm =
        sqrt(s->d[0] * s->d[0] + s->d[1] * s->d[1] + s->d[2] * s->d[2]);
    if (norm == 0.)
...
static void gfs_source_coriolis_write(GtsObject* o, FILE* fp) {
  ...
  if (s->drag) gfs_function_write(s->drag, fp);
  if (s->d[2] != 1.)
    fprintf(fp, " { x = %g y = %g z = %g }", s->d[0], s->d[1], s->d[2]);
}

static void coriolis_drag_metric_coefficients(
    GfsSourceCoriolis* sc, FttCell* cell, GfsVariable** u, FttComponent c1,
    gdouble f[2]) {
...
static gdouble gfs_source_coriolis_mac_value(
    GfsSourceGeneric* s, FttCell* cell, GfsVariable* v) {
  ...
  switch (v->component) {
    case FTT_X:
      return f[1] * GFS_VALUE(cell, sv->v[1]) -
             f[0] * GFS_VALUE(cell, sv->v[0]);
    case FTT_Y:
      return -f[0] * GFS_VALUE(cell, sv->v[1]) -
             f[1] * GFS_VALUE(cell, sv->v[0]);
    default:
  ...
  switch (v->component) {
    case FTT_X:
      return -e * GFS_VALUE(cell, sv->v[0]) +
             f * (GFS_VALUE(cell, sv->v[1]) * sc->d[2] -
                  GFS_VALUE(cell, sv->v[2]) * sc->d[1]);
    case FTT_Y:
      return -e * GFS_VALUE(cell, sv->v[1]) +
             f * (GFS_VALUE(cell, sv->v[2]) * sc->d[0] -
                  GFS_VALUE(cell, sv->v[0]) * sc->d[2]);
    case FTT_Z:
      return -e * GFS_VALUE(cell, sv->v[2]) +
             f * (GFS_VALUE(cell, sv->v[0]) * sc->d[1] -
                  GFS_VALUE(cell, sv->v[1]) * sc->d[0]);
    default:
      g_assert_not_reached();
  }
#endif /* 3D */
...
static void save_coriolis(FttCell* cell, GfsSourceCoriolis* s) {
  ...
  GFS_VALUE(cell, s->u[0]) =
      (1. - s->beta) *
      (f[1] * GFS_VALUE(cell, sv->v[1]) - f[0] * GFS_VALUE(cell, sv->v[0]));
  coriolis_drag_metric_coefficients(s, cell, sv->v, FTT_Y, f);
  GFS_VALUE(cell, s->u[1]) =
      (1. - s->beta) *
      (-f[0] * GFS_VALUE(cell, sv->v[1]) - f[1] * GFS_VALUE(cell, sv->v[0]));
#else /* 3D */
  ...
  GFS_VALUE(cell, s->u[0]) = -e * GFS_VALUE(cell, sv->v[0]) +
                             f * (GFS_VALUE(cell, sv->v[1]) * s->d[2] -
                                  GFS_VALUE(cell, sv->v[2]) * s->d[1]);
  GFS_VALUE(cell, s->u[1]) = -e * GFS_VALUE(cell, sv->v[1]) +
                             f * (GFS_VALUE(cell, sv->v[2]) * s->d[0] -
                                  GFS_VALUE(cell, sv->v[0]) * s->d[2]);
  GFS_VALUE(cell, s->u[2]) = -e * GFS_VALUE(cell, sv->v[2]) +
                             f * (GFS_VALUE(cell, sv->v[0]) * s->d[1] -
                                  GFS_VALUE(cell, sv->v[1]) * s->d[0]);
#endif /* 3D */
}
  ...
  if ((*GFS_EVENT_CLASS(GTS_OBJECT_CLASS(gfs_event_sum_class())->parent_class)
    ...
    if (GFS_SOURCE_CORIOLIS(event)->beta < 1.) {
      ...
      gfs_domain_traverse_layers(
          GFS_DOMAIN(sim), (FttCellTraverseFunc)save_coriolis, event);
...
static gdouble gfs_source_coriolis_centered_value(
    GfsSourceGeneric* s, FttCell* cell, GfsVariable* v) {
...
GfsSourceGenericClass* gfs_source_coriolis_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo gfs_source_coriolis_info = {
        ...
        (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_source_velocity_class()),
...
GfsSourceCoriolis* gfs_has_source_coriolis(GfsDomain* domain) {
  ...
  if (v->sources) {
    ...
    while (i) {
      if (GFS_IS_SOURCE_CORIOLIS(i->data)) return i->data;
      i = i->next;
    }
  }
  return NULL;
...
static void implicit_coriolis_3D(FttCell* cell, GfsSourceCoriolis* s) {
  ...
  GfsSimulation* sim = gfs_object_simulation(s);
  gdouble c =
      sim->advection_params.dt * gfs_function_value(s->omegaz, cell) * s->beta;
  gdouble e = s->drag ? sim->advection_params.dt *
                            gfs_function_value(s->drag, cell) * s->beta
                      : 0.;
  GtsMatrix* m = gts_matrix_new(
      1. + e, -c * s->d[2], c * s->d[1], 0., c * s->d[2], 1. + e, -c * s->d[0],
      0., -c * s->d[1], c * s->d[0], 1. + e, 0., 0., 0., 0., 0.);
...
void gfs_source_coriolis_implicit(GfsDomain* domain, gdouble dt) {
  ...
  if ((s = gfs_has_source_coriolis(domain))) {
    ...
    gfs_domain_cell_traverse(
        domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
        (FttCellTraverseFunc)(
            s->d[2] != 1. ? implicit_coriolis_3D : implicit_coriolis_2D),
        s);
    if (gfs_restore_floating_point_exceptions()) {
      gchar* c =
