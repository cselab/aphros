...
static void gfs_adapt_read(GtsObject** o, GtsFile* fp) {
  ...
  while (fp->type != GTS_ERROR && fp->type != '}') {
    ...
    if (fp->type != GTS_STRING) {
      ...
      a->c = gfs_domain_get_or_add_variable(
          domain, fp->token->str, "Adaptive refinement cost");
      if (!a->c) {
...
static void gfs_adapt_write(GtsObject* o, FILE* fp) {
  ...
  fputc(' ', fp);
  if (a->mincells > 0) fprintf(fp, "mincells = %u ", a->mincells);
  if (a->maxcells < G_MAXINT) fprintf(fp, "maxcells = %u ", a->maxcells);
  if (a->cmax > 0.) fprintf(fp, "cmax = %g ", a->cmax);
  if (a->weight != 1.) fprintf(fp, "weight = %g ", a->weight);
  if (a->cfactor != 4.) fprintf(fp, "cfactor = %g ", a->cfactor);
  if (a->c != NULL) fprintf(fp, "c = %s ", a->c->name);
  fputc('}', fp);
}
...
static void gfs_adapt_post_event(GfsEvent* event, GfsSimulation* sim) {
  ...
  if (a->active && a->c)
    gfs_domain_cell_traverse(
        GFS_DOMAIN(sim), FTT_PRE_ORDER, FTT_TRAVERSE_ALL, -1,
...
GfsEventClass* gfs_adapt_class(void) {
  ...
  if (klass == NULL) {
    ...
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_event_class()), &gfs_adapt_info);
  }
...
GfsEventClass* gfs_adapt_vorticity_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo gfs_adapt_vorticity_info = {
        ...
        (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_adapt_class()), &gfs_adapt_vorticity_info);
  }
...
GfsEventClass* gfs_adapt_streamline_curvature_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo gfs_adapt_streamline_curvature_info = {
        ...
        (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_adapt_class()),
...
GfsEventClass* gfs_adapt_function_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo gfs_adapt_function_info = {
        ...
        (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_adapt_class()), &gfs_adapt_function_info);
  }
...
GfsEventClass* gfs_adapt_gradient_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo gfs_adapt_gradient_info = {
        ...
        (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_adapt_function_class()), &gfs_adapt_gradient_info);
  }
...
static gboolean gfs_adapt_error_event(GfsEvent* event, GfsSimulation* sim) {
  if ((*GFS_EVENT_CLASS(GTS_OBJECT_CLASS(gfs_adapt_error_class())->parent_class)
    ...
    gfs_domain_cell_traverse(
        domain, FTT_PRE_ORDER, FTT_TRAVERSE_ALL, -1, (FttCellTraverseFunc)scale,
        a);
    for (a->c = 0; a->c < FTT_DIMENSION; a->c++)
...
GfsEventClass* gfs_adapt_error_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo gfs_adapt_error_info = {
        ...
        (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_adapt_gradient_class()), &gfs_adapt_error_info);
  }
...
static FttCell* remove_top_coarse(
    GtsEHeap* h, gdouble* cost, GfsVariable* hcoarse) {
  FttCell* cell = gts_eheap_remove_top(h, cost);

  if (cell) GFS_VALUE(cell, hcoarse) = 0.;
...
static FttCell* remove_top_fine(
    GtsEHeap* h, gdouble* cost, GfsVariable* hfine) {
  FttCell* cell = gts_eheap_remove_top(h, cost);

  if (cell) GFS_VALUE(cell, hfine) = 0.;
...
static gdouble refine_cost(FttCell* cell, GfsSimulation* sim) {
  ...
  while (i) {
    ...
    if (a->active && a->cost) cost += a->weight * (*a->cost)(cell, a);
    i = i->next;
  }
...
static gboolean fine_cell_coarsenable(FttCell* cell, AdaptParams* p) {
  if (GFS_CELL_IS_BOUNDARY(cell)) return TRUE;
  if (GFS_CELL_IS_PERMANENT(cell)) return FALSE;
  if (CELL_COST(cell) >= -p->clim) return FALSE;
  if (ftt_cell_level(cell) < minlevel(cell, p->sim)) return FALSE;
  if (ftt_refine_corner(cell)) return FALSE;
  return TRUE;
}
...
static void fine_cell_cleanup(FttCell* cell, AdaptParams* p) {
  if (!GFS_CELL_IS_BOUNDARY(cell)) {
    ...
    p->nc--;
    if ((o = CELL_HCOARSE(cell))) gts_eheap_remove(p->hcoarse, o);
    if ((o = CELL_HFINE(cell))) gts_eheap_remove(p->hfine, o);
  }
...
static void cell_fine_init(FttCell* cell, AdaptParams* p) {
  ...
  for (n = 0; n < FTT_CELLS; n++)
    if (child.c[n]) CELL_COST(child.c[n]) = G_MAXDOUBLE;
  if (!GFS_CELL_IS_BOUNDARY(cell)) p->nc += FTT_CELLS;
}

static gboolean adapt_global(
    GfsSimulation* simulation, guint* depth, GfsAdaptStats* s, guint mincells,
    guint maxcells, GfsVariable* c, gdouble cmax) {
  ...
  while (changed) {
    ...
    if (fine && ((cfine < -ccoarse && apar.nc > maxcells) ||
      ...
      ftt_cell_coarsen(
          fine, (FttCellCoarsenFunc)fine_cell_coarsenable, &apar,
...
static gboolean coarsen_cell(FttCell* cell, AdaptLocalParams* p) {
  if (GFS_CELL_IS_BOUNDARY(cell)) return COARSENABLE(cell, p);
  if (ftt_refine_corner(cell)) return FALSE;
...
static void coarsen_box(GfsBox* box, AdaptLocalParams* p) {
  ftt_cell_coarsen(
      box->root, (FttCellCoarsenFunc)coarsen_cell, p,
  ...
  for (d = 0; d < FTT_NEIGHBORS; d++)
    if (GFS_IS_BOUNDARY_PERIODIC(box->neighbor[d])) {
      ...
      ftt_face_traverse_boundary(
          b->root, b->d, FTT_PRE_ORDER, FTT_TRAVERSE_NON_LEAFS, -1,
          (FttFaceTraverseFunc)check_periodic, p);
    }
}

static gboolean adapt_local(
    GfsSimulation* sim, guint* depth, GfsAdaptStats* s) {
  GfsDomain* domain = GFS_DOMAIN(sim);
  AdaptLocalParams p;
  p.sim = sim;
  p.depth = *depth;
...
gboolean gfs_simulation_adapt(GfsSimulation* simulation) {
  ...
  if (active) {
    ...
    if (maxcells < G_MAXINT)
      changed = adapt_global(
          simulation, &depth, &simulation->adapts_stats, mincells, maxcells, c,
          cmax);
    else
      ...
      while (i) {
        if (GTS_OBJECT(i->data)->klass ==
          ...
          gfs_hydrostatic_pressure_update(
              i->data, simulation->physical_params.alpha);
        i = i->next;
      }
    }
  }
