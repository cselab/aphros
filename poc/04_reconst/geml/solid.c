...
void gfs_cell_fluid(FttCell* cell) {
  ...
  if (!FTT_CELL_IS_LEAF(cell)) {
    ...
    for (i = 0; i < FTT_CELLS; i++)
      if (child.c[i]) gfs_cell_fluid(child.c[i]);
  }
}
...
static void face_new(
    CellFace* f, FttCell* cell, GfsGenericSurface* s, FttVector* h) {
  FttVector p;
  guint i;
...
static gboolean solid_face_is_thin(CellFace* f) {
  ...
  if (odd == 2 && even == 1) {
    for (i = 0; i < 4; i++)
      if (f->s[i].n % 2 != 0 && f->s[(i + 2) % 4].n % 2 != 0) return FALSE;
    return TRUE;
  }
  return (odd > 2 || even > 1);
}
...
gboolean gfs_set_2D_solid_fractions_from_surface(
    FttCell* cell, GfsGenericSurface* s) {
  ...
  switch (n1) {
    ...
    default: {
      ...
      ftt_cell_pos(cell, &p);
      g_log(
          G_LOG_DOMAIN, G_LOG_LEVEL_ERROR,
...
static void set_solid_fractions_from_surface(
    FttCell* cell, GfsGenericSurface* s, InitSolidParams* p) {
...
static void cube_new(
    CellCube* cube, FttCell* cell, GfsGenericSurface* s, FttVector* o,
    FttVector* h) {
  guint i;
...
static void set_solid_fractions_from_surface(
    FttCell* cell, GfsGenericSurface* surface, InitSolidParams* p) {
  GfsSolidVector* solid = GFS_STATE(cell)->solid;
  CellCube cube;
  ...
  if (solid->a == 0.) GFS_VALUE(cell, p->status) = GFS_STATUS_SOLID;
}
...
gboolean gfs_solid_is_thin(FttCell* cell, GfsGenericSurface* s) {
  ...
  for (i = 0; i < FTT_NEIGHBORS; i++) {
    ...
    if (solid_face_is_thin(&f)) return TRUE;
  }
...
static void push_leaf(
    GtsFifo* fifo, FttCell* cell, FttDirection d, gdouble a,
    GfsVariable* status) {
  if (FTT_CELL_IS_LEAF(cell)) {
    ...
    for (i = 0; i < FTT_NEIGHBORS; i++)
      if ((n = ftt_cell_neighbor(cell, i)) && !GFS_CELL_IS_BOUNDARY(n)) {
        if (solid->s[i] == 0. || solid->s[i] == 1.) {
          ...
          if (w / k <= 0. || w / k >= 1.)
            g_warning(
                "file %s: line %d (%s): w/k=%g solid->s[%d]=%g", __FILE__,
                __LINE__, G_GNUC_PRETTY_FUNCTION, w / k, i, solid->s[i]);
          solid->s[i] = w / k;
        }
      }
...
static void solid_fractions_from_children(FttCell* cell, InitSolidParams* p) {
  if (!FTT_CELL_IS_LEAF(cell)) {
    ...
    ftt_cell_children(cell, &child);
    for (i = 0; i < FTT_CELLS; i++)
      if (child.c[i]) solid_fractions_from_children(child.c[i], p);
    if (FTT_CELL_IS_LEAF(cell))
...
guint gfs_init_solid_fractions_leaves(
    GfsDomain* domain, GSList* i, GfsVariable* status) {
  InitSolidParams p;
  ...
  if (status == NULL) gts_object_destroy(GTS_OBJECT(p.status));
...
void gfs_init_solid_fractions_from_children(
    GfsDomain* domain, gboolean destroy_solid, FttCellCleanupFunc cleanup,
...
guint gfs_domain_init_solid_fractions(
    GfsDomain* domain, GSList* i, gboolean destroy_solid,
    FttCellCleanupFunc cleanup, gpointer data, GfsVariable* status) {
  GfsVariable* status1;
  ...
  gfs_init_solid_fractions_from_children(
      domain, destroy_solid, cleanup, data, status1);
  if (status == NULL) gts_object_destroy(GTS_OBJECT(status1));
...
static gboolean check_area_fractions(const FttCell* root) {
  ...
  if (solid) {
    ...
    if (!gts_bbox_point_is_inside(&bb, &solid->cm)) {
      g_warning(
          "file %s: line %d (%s): cm (%g,%g,%g)/%d is not inside cell "
          "[(%g,%g,%g),(%g,%g,%g)]",
          __FILE__, __LINE__, G_GNUC_PRETTY_FUNCTION, solid->cm.x, solid->cm.y,
          solid->cm.z, ftt_cell_level(root), bb.x1, bb.y1, bb.z1, bb.x2, bb.y2,
          bb.z2);
      ret = FALSE;
      g_assert_not_reached();
    }
    if (!gts_bbox_point_is_inside(&bb, &solid->ca)) {
      g_warning(
          "file %s: line %d (%s): ca (%g,%g,%g)/%d is not inside cell "
          "[(%g,%g,%g),(%g,%g,%g)]",
          __FILE__, __LINE__, G_GNUC_PRETTY_FUNCTION, solid->ca.x, solid->ca.y,
          solid->ca.z, ftt_cell_level(root), bb.x1, bb.y1, bb.z1, bb.x2, bb.y2,
          bb.z2);
      ret = FALSE;
      g_assert_not_reached();
    }
        ...
        if (GFS_IS_FLUID(root)) {
          if (!GFS_IS_FLUID(neighbor.c[i])) {
            if (1. - nsolid->s[oi] >= 1e-10) {
              ...
              ftt_cell_pos(root, &p);
              g_warning(
                  "file %s: line %d (%s): (%g,%g,%g)/%d: s[%d]: %g", __FILE__,
                  __LINE__, G_GNUC_PRETTY_FUNCTION, p.x, p.y, p.z,
          ...
          if (fabs(solid->s[i] - nsolid->s[oi]) >= 1e-10) {
            ...
            ftt_cell_pos(root, &p);
            g_warning(
                "file %s: line %d (%s): (%g,%g,%g)/%d: s[%d]: %g "
                "neighbor->s[%d]: %g",
                __FILE__, __LINE__, G_GNUC_PRETTY_FUNCTION, p.x, p.y, p.z,
  ...
  if (!FTT_CELL_IS_LEAF(root)) {
    ...
    for (i = 0; i < FTT_CELLS; i++)
      if (child.c[i] && !check_area_fractions(child.c[i])) ret = FALSE;
  }
  ...
  if (!GFS_IS_MIXED(cell)) {
    for (n = 0; n < FTT_CELLS; n++)
      if (children.c[n] && GFS_IS_MIXED(children.c[n])) {
        g_warning(
            "file %s: line %d (%s): children[%d] is mixed (%g)"
            " parent is not",
            __FILE__, __LINE__, G_GNUC_PRETTY_FUNCTION, n,
    ...
    if (fabs(GFS_STATE(cell)->solid->a - a) >= 1e-10) {
      g_warning(
          "file %s: line %d (%s): children->a: %g parent->a: %g", __FILE__,
          __LINE__, G_GNUC_PRETTY_FUNCTION, a, GFS_STATE(cell)->solid->a);
      *ret = FALSE;
    }
  }
}
...
gboolean gfs_cell_check_solid_fractions(FttCell* root) {
  ...
  ftt_cell_traverse(
      root, FTT_POST_ORDER, FTT_TRAVERSE_NON_LEAFS, -1,
...
static void restore_solid(FttCell* cell, gpointer* data) {
  ...
  if (solid) {
    ...
    g_assert(
        GFS_VALUE(cell, status) == GFS_STATUS_SOLID ||
...
static void check_status(GfsBox* box, gpointer* data) {
  ...
  if (!GFS_IS_MIXED(box->root) &&
    ...
    data[2] = &val;
    ftt_cell_traverse(
        box->root, FTT_PRE_ORDER, FTT_TRAVERSE_ALL, -1,
...
void gfs_domain_init_fraction(
    GfsDomain* domain, GfsGenericSurface* s, GfsVariable* c) {
  gpointer data[3];
  GfsVariable* status;
...
static void outer_fractions_coarse_fine(FttCell* parent, FttDirection d) {
  ...
  if (fabs(solid->s[d] - ss / n) > 1e-5)
    g_warning(
        "inconsistent surface fractions %d %f %f %f\n", d, solid->s[d], ss / n,
        fabs(solid->s[d] - ss / n));
}
#endif /* 3D */
      ...
      for (d = 0; d < FTT_NEIGHBORS; d++)
        if (!n.c[d])
          ...
          s->s[d] = GFS_STATE(n.c[d])->solid->s[FTT_OPPOSITE_DIRECTION(d)];
            ...
            !ftt_cell_neighbor_is_brother(child.c[i], d) &&
            GFS_IS_FLUID(n.c[d]))
          s->s[d] = 1.;
        else {
#if FTT_2D
          ...
          if (ftt_cell_neighbor_is_brother(child.c[i], d)) {
            ...
            mp.y = (&m.x)[c2];
            s->s[d] = gfs_line_area(
                &mp, d % 2 ? 2. * alpha1 : 2. * alpha1 - (&m.x)[d / 2]);
          }
#endif /* 3D */
        }
    } else if (a == 0.)
      ftt_cell_destroy(
          child.c[i], (FttCellCleanupFunc)gfs_cell_cleanup, domain);
  }
...
GfsEventClass* gfs_solid_class(void) {
  ...
  if (klass == NULL) {
    ...
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_event_class()), &gfs_solid_info);
  }
