 ...
 */
void gfs_line_center(
    const FttVector* m, gdouble alpha, gdouble a, FttVector* p) {
  FttVector n;
  gdouble b;
...
gdouble gfs_plane_area_center(const FttVector* m, gdouble alpha, FttVector* p) {
  ...
  return area *
         sqrt(
             1. / (n.x * n.x * n.y * n.y) + 1. / (n.x * n.x * n.z * n.z) +
             1. / (n.z * n.z * n.y * n.y)) /
         6.;
}
#endif /* 3D */
 ...
 */
gdouble gfs_vof_plane_interpolate(
    FttCell* cell, FttVector* p, guint level, GfsVariableTracerVOF* t,
    FttVector* m) {
...
static void variable_tracer_vof_update(GfsVariable* v, GfsDomain* domain) {
  ...
  for (l = 0; l <= depth; l++) {
    gfs_domain_cell_traverse(
        domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEVEL, l,
...
static void variable_tracer_vof_read(GtsObject** o, GtsFile* fp) {
  (*GTS_OBJECT_CLASS(gfs_variable_tracer_vof_class())->parent_class->read)(
      o, fp);
  if (fp->type == GTS_ERROR) return;

  if (GFS_VARIABLE_TRACER(*o)->advection.cfl > 0.5) {
    gts_file_error(
        fp, "cfl `%g' is out of range `]0,0.5]'",
...
static void vof_coarse_fine(FttCell* parent, GfsVariable* v) {
  ...
  if (GFS_IS_FULL(f)) {
    ...
    while (j) {
      ...
      for (i = 0; i < FTT_CELLS; i++)
        if (child.c[i]) GFS_VALUE(child.c[i], v) = GFS_VALUE(parent, v);
      j = j->next;
    }
  }
}
...
GfsVariableTracerVOFClass* gfs_variable_tracer_vof_class(void) {
  ...
  if (klass == NULL) {
    ...
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_variable_tracer_class()), &info);
  }
...
static gdouble plane_volume_shifted(
    FttVector m, gdouble alpha, FttVector p[2]) {
  FttComponent c;
...
static gdouble coarse_fraction(
    FttCellFace* face, GfsVariable* v, gdouble un, FttVector q[2]) {
...
static void face_too_coarse(FttCellFace* face, VofParms* p) {
  if (ftt_face_type(face) == FTT_FINE_COARSE) {
    ...
    if (!FTT_FACE_DIRECT(face)) un = -un;
    if (un > 0.) {
      gdouble f = GFS_VALUE(face->neighbor, p->par->v);
      if (GFS_IS_FULL(f)) {
        FttVector q[2] = {{0., 0., 0.}, {1., 1., 1.}};
        if (fine_fraction(
                face, p->vof, un * p->par->dt / ftt_cell_size(face->cell), q) !=
            f) {
          p->too_coarse++;
...
static void vof_cell_fine_init_with_streamfunction(
    FttCell* parent, VofParms* p) {
  gfs_cell_fine_init(parent, p->domain);
...
static void refine_too_coarse(FttCell* cell, VofParms* p) {
  if (TOO_COARSE(cell)) {
    ...
    if (level + 1 > p->depth) p->depth = level + 1;
  }
}
...
static void fix_too_coarse(GfsDomain* domain, VofParms* p) {
  ...
  gfs_domain_face_traverse(
      domain, p->c, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
...
#endif /* 2D */
  domain->cell_init = (FttCellInitFunc)(
  ...
  if (p->too_coarse > 0) gfs_domain_reshape(domain, p->depth);
...
static void concentration_face_values(FttCell* cell, VofParms* p) {
  ...
  if (p->sink) unorm += p->par->dt * gfs_function_value(p->sink, cell) / size;
...
static void vof_flux(FttCellFace* face, VofParms* p) {
  ...
  gdouble un = GFS_FACE_NORMAL_VELOCITY(face) * p->par->dt / size,
          dun[FTT_DIMENSION - 1];
  if (p->sink) un += gfs_function_face_value(p->sink, face) * p->par->dt / size;
  FttComponent c;
  ...
  for (i = 0; i < n; i++) {
    ...
    if (p->par->v == p->vof) GFS_VALUE(face->cell, p->vpar.fv) -= uni * f;
...
void gfs_tracer_vof_advection(GfsDomain* domain, GfsAdvectionParams* par) {
  ...
  gfs_domain_traverse_leaves(
      domain, (FttCellTraverseFunc)initialize_dV, p.vpar.v);
  ...
  while (j) {
    ...
    gfs_domain_traverse_leaves(
        domain, (FttCellTraverseFunc)per_vof_volume, j->data);
    j = j->next;
  }
  for (c = 0; c < FTT_DIMENSION; c++) {
    ...
    while (j) {
      ...
      if (par->sink[0]) {
        p.sink = par->sink[p.c];
        gfs_domain_traverse_leaves(
            domain, (FttCellTraverseFunc)add_sink_velocity, &p);
        gfs_domain_traverse_leaves(domain, (FttCellTraverseFunc)grad_u, &p);
        for (d = 0; d < FTT_DIMENSION - 1; d++)
          gfs_domain_bc(domain, FTT_TRAVERSE_LEAFS, -1, p.du[d]);
      }
      ...
      gfs_domain_traverse_merged(
          domain, (GfsMergedTraverseFunc)par->update, par);
      p.par->fv = fv;
      p.par->v = p.vof;
      j = j->next;
    }
    ...
    gfs_domain_traverse_merged(
        domain, (GfsMergedTraverseFunc)par->update, &p.vpar);
  ...
  while (j) {
    ...
    gfs_domain_traverse_leaves(
        domain, (FttCellTraverseFunc)per_cell_volume, j->data);
    j = j->next;
  }
 ...
 */
guint gfs_vof_facet(
    FttCell* cell, GfsVariableTracerVOF* t, FttVector* p, FttVector* m) {
  g_return_val_if_fail(cell != NULL, 0);
  g_return_val_if_fail(t != NULL, 0);
  g_return_val_if_fail(p != NULL, 0);
  g_return_val_if_fail(m != NULL, 0);

  if (GFS_IS_FULL(GFS_VALUE(cell, GFS_VARIABLE(t)))) return 0;
 ...
 */
gdouble gfs_vof_facet_distance2(
    FttCell* cell, GfsVariableTracerVOF* t, GtsPoint* p) {
  g_return_val_if_fail(cell != NULL, GFS_NODATA);
  g_return_val_if_fail(t != NULL, GFS_NODATA);
  g_return_val_if_fail(p != NULL, GFS_NODATA);

  if (GFS_IS_FULL(GFS_VALUE(cell, GFS_VARIABLE(t)))) return GFS_NODATA;
...
static gdouble local_height(
    FttVector* p, FttVector* origin, guint level, GfsVariable* v,
    FttDirection d, GtsVector interface) {
  ...
  while (n < NMAX && (!found_interface || !GFS_IS_FULL(right))) {
    ...
    if (right > 1.) return G_MAXDOUBLE;
    ADD_H(right);
    if (!GFS_IS_FULL(right)) found_interface = TRUE;
  }
  if (right != 1.) return G_MAXDOUBLE;
  ...
  while (n < NMAX && (!found_interface || !GFS_IS_FULL(left))) {
    ...
    if (left > 1.) return G_MAXDOUBLE;
    ADD_H(left);
    if (!GFS_IS_FULL(left)) found_interface = TRUE;
  }
  if (left != 0.) return G_MAXDOUBLE;
...
static gboolean curvature_along_direction(
    FttCell* cell, GfsVariableTracerVOF* t, FttComponent c, gdouble* kappa,
    gdouble* kmax, GtsVector* interface, guint* n) {
  GfsVariable* v = GFS_VARIABLE(t);
  ...
  if (found_all_heights) {
    ...
    *kappa = hxx / (size * sqrt(dnm * dnm * dnm));
    ...
    if (GFS_IS_AXI(v->domain)) {
      ...
      *kappa += kaxi;
      if (kmax) *kmax = MAX(*kmax, fabs(kaxi));
    }
  }
#else /* 3D */
    static FttComponent or
        [3][2] = {{FTT_Y, FTT_Z}, {FTT_X, FTT_Z}, {FTT_X, FTT_Y}};
    gdouble h[3][3];
    gint x, y;
...
static gdouble parabola_fit_curvature(
    ParabolaFit* p, gdouble kappamax, gdouble* kmax) {
  gdouble kappa;
#if FTT_2D
...
#if FTT_2D
static void parabola_fit_axi_curvature(
    const ParabolaFit* p, gdouble r, gdouble h, gdouble* kappa, gdouble* kmax) {
...
static void add_vof_center(
    FttCell* cell, FttVector* p, guint level, FttVector* origin,
    GfsVariableTracerVOF* t, ParabolaFit* fit, gdouble w) {
...
static void fit_from_fractions(
    FttCell* cell, GfsVariable* v, ParabolaFit* fit) {
  gdouble h = ftt_cell_size(cell);
  guint level = ftt_cell_level(cell);
  gint x, y, z = 0;
  FttVector p;
    ...
    for (x = -1; x <= 1; x++)
      for (y = -1; y <= 1; y++)
        if (x != 0 || y != 0 || z != 0) {
          ...
          if (neighbor)
            add_vof_center(
                neighbor, &o, level, &p, GFS_VARIABLE_TRACER_VOF(v), fit, 1.);
        }
}
 ...
 * @cell.
 */
gdouble gfs_fit_curvature(
    FttCell* cell, GfsVariableTracerVOF* t, gdouble* kmax) {
  g_return_val_if_fail(cell != NULL, 0.);
  g_return_val_if_fail(t != NULL, 0.);
...
static gdouble neighboring_column(
    FttCell* cell, GfsVariable* h, FttComponent c, gdouble orientation,
    FttDirection d, gdouble* x) {
  FttCell* n = ftt_cell_neighbor(cell, d);
  if (!n) return GFS_NODATA;
...
static void curvature_from_h(
    FttCell* cell, GfsDomain* domain, gdouble x[3], gdouble h[3],
    gdouble orientation, FttComponent c, gdouble* kappa, gdouble* kmax) {
  gdouble size = ftt_cell_size(cell);
 ...
 * otherwise.
 */
gboolean gfs_curvature_along_direction(
    FttCell* cell, GfsVariableTracerVOFHeight* t, FttComponent c,
  ...
  if (h[0] != GFS_NODATA && x[0] == 1.) {
    ...
    if (h[1] != GFS_NODATA && x[1] == 1.) {
      x[1] = -x[1];
      curvature_from_h(
          cell, GFS_VARIABLE(t)->domain, x, h, orientation, c, kappa, kmax);
      return TRUE;
    }
  }
#else /* 3D */
...
static gboolean curvature_along_direction_new(
    FttCell* cell, GfsVariableTracerVOFHeight* t, FttComponent c,
    gdouble* kappa, gdouble* kmax, GtsVector* interface, guint* nb) {
#ifdef FTT_2D
  gdouble orientation;
  GfsVariable* hv = gfs_closest_height(cell, t, c, &orientation);
  FttComponent oc = FTT_ORTHOGONAL_COMPONENT(c);
  if (!hv) {
    ...
    if (!hv)
      hv = gfs_closest_height(
          ftt_cell_neighbor(cell, 2 * oc + 1), t, c, &orientation);
    if (!hv) /* give up */
      return FALSE;
  } else if (fabs(GFS_VALUE(cell, hv)) > 1.)
    return FALSE; /* interface is too far */
 ...
 * compute a consistent curvature.
 */
gdouble gfs_height_curvature_new(
    FttCell* cell, GfsVariableTracerVOFHeight* t, gdouble* kmax) {
  g_return_val_if_fail(cell != NULL, 0.);
  g_return_val_if_fail(t != NULL, 0.);
  ...
  guint n = 0;
  for (c = 0; c < FTT_DIMENSION; c++) /* try each direction */
    if (curvature_along_direction_new(
            cell, t, try[c], &kappa, kmax, interface, &n))
      return kappa;
...
gdouble gfs_vof_correctness(FttCell* cell, GfsVariableTracerVOF* t) {
  ...
  if (GFS_VALUE(cell, v) <= 0. || GFS_VALUE(cell, v) >= 1.) return 1.;
...
static void variable_vof_concentration_write(GtsObject* o, FILE* fp) {
  ...
  fprintf(
      fp, " %s", GFS_VARIABLE(GFS_VARIABLE_VOF_CONCENTRATION(o)->vof)->name);
}
...
static gint children_half_height(
    FttCell* cell, FttDirection d, GfsVariable* fv, gdouble* H, gint* n) {
  FttCellChildren child;
  guint i,
...
static gint half_height(
    FttCell* cell, GfsVariable* fv, FttDirection d, gdouble* H, gint* n) {
  gint s = 0;
  *n = 0;
  guint level = ftt_cell_level(cell);
  ...
  while (*n < HMAX && !s && neighbor) {
    ...
    if (f > 0. && f < 1.) { /* interfacial cell */
      if (ftt_cell_level(neighbor) < level) /* neighbor is coarser */
        return 0;
      if (GFS_CELL_IS_BOUNDARY(neighbor)) return 2;
...
static void height_propagation(
    FttCell* cell, HFState* hf, GfsVariable* h, gdouble orientation) {
  guint level = ftt_cell_level(cell);
  FttDirection d;
  ...
  if (!FTT_CELL_IS_LEAF(cell)) {
    ...
    for (d = 2 * c; d <= 2 * c + 1; d++) {
      ...
      for (i = 0; i < n; i++)
        if (child.c[i]) {
          if (h == NULL)
            h = gfs_closest_height(
                child.c[i], GFS_VARIABLE_TRACER_VOF_HEIGHT(hf->f), hf->c,
                &orientation);
...
static void height_propagation_from_boundary(
    FttCell* cell, HFState* hf, GfsVariable* h) {
  guint level = ftt_cell_level(cell);
  ...
  for (hf->d = 2 * hf->c; hf->d <= 2 * hf->c + 1; hf->d++)
    if (GFS_IS_BOUNDARY_PERIODIC(box->neighbor[hf->d]))
      ftt_cell_traverse_boundary(
          box->root, hf->d, FTT_PRE_ORDER, FTT_TRAVERSE_ALL, -1,
...
static void box_contact_bc(GfsBox* box, HFState* hf) {
  ...
  for (hf->d = 0; hf->d < FTT_NEIGHBORS; hf->d++)
    if (GFS_IS_BOUNDARY(box->neighbor[hf->d]) &&
      ...
      ftt_cell_traverse_boundary(
          box->root, hf->d, FTT_POST_ORDER, FTT_TRAVERSE_ALL, -1, contact_bc,
          hf);
    }
}
...
static void remaining_boundary_height_undefined(FttCell* cell, HFState* hf) {
  GfsVariable* h = boundary_hit(cell, hf);
  if (h) GFS_VALUE(cell, h) = GFS_NODATA;
}
...
static gboolean height_normal(FttCell* cell, GfsVariable* v, FttVector* m) {
  ...
  for (c = 0; c < 2; c++) {
    ...
    if (hv != NULL && fabs(GFS_VALUE(cell, hv)) <= 1.) {
      ...
      FttComponent oc = FTT_ORTHOGONAL_COMPONENT(c);
      h[0] = neighboring_column(cell, hv, c, orientation, 2 * oc, &x[0]);
      if (h[0] == GFS_NODATA) continue;
      h[1] = neighboring_column(cell, hv, c, orientation, 2 * oc + 1, &x[1]);
      if (h[1] == GFS_NODATA) continue;
      x[1] = -x[1];
...
static void vof_height_plane(FttCell* cell, GfsVariable* v) {
  if (FTT_CELL_IS_LEAF(cell)) {
    ...
    if (GFS_IS_FULL(f)) {
      ...
      if (!height_normal(cell, v, &m)) myc_normal(cell, v, &m);
      for (c = 0; c < FTT_DIMENSION; c++)
        n += fabs((&m.x)[c]);
      if (n > 0.)
        for (c = 0; c < FTT_DIMENSION; c++)
...
static void variable_tracer_vof_height_update(
    GfsVariable* v, GfsDomain* domain) {
  gfs_domain_cell_traverse(
      domain, FTT_POST_ORDER, FTT_TRAVERSE_NON_LEAFS, -1,
      (FttCellTraverseFunc)v->fine_coarse, v);
  gfs_domain_bc(domain, FTT_TRAVERSE_ALL, -1, v);
  ...
  for (hf.c = 0; hf.c < FTT_DIMENSION; hf.c++) {
    ...
    gfs_domain_cell_traverse_condition(
        domain, FTT_PRE_ORDER, FTT_TRAVERSE_ALL, -1,
        (FttCellTraverseFunc)height, &hf, is_interfacial, hf.f);
...
static void undefined_coarse_fine(FttCell* parent, GfsVariable* v) {
  ...
  ftt_cell_children(parent, &child);
  for (i = 0; i < FTT_CELLS; i++)
    if (child.c[i]) GFS_VALUE(child.c[i], v) = GFS_NODATA;
}
...
static void variable_tracer_vof_height_read(GtsObject** o, GtsFile* fp) {
  ...
  for (c = 0; c < FTT_DIMENSION; c++) {
    ...
    gchar* description = g_strdup_printf(
        "%s-component (bottom) of the height function for the interface "
        "defined by %s",
        index[c], v->name);
    ...
    description = g_strdup_printf(
        "%s-component (top) of the height function for the interface defined "
        "by %s",
        index[c], v->name);
 ...
 */
GfsVariable* gfs_closest_height(
    FttCell* cell, GfsVariableTracerVOFHeight* t, FttComponent c,
    gdouble* orientation) {
...
gdouble gfs_vof_face_fraction(
    const FttCellFace* face, GfsVariableTracerVOF* t) {
  g_return_val_if_fail(face != NULL, 0.);
  g_return_val_if_fail(t != NULL, 0.);
