...
int main(int argc, char* argv[]) {
  ...
  MPI_Allgather(
      &c_hash, 1, MPI_UINT64_T, all_hashes.data(), 1, MPI_UINT64_T,
      comm_hypre_);
  ...
  if (HasHyperthreads()) {
    ...
    for (size_t i = 0; i < thread_affinity.size(); ++i) {
      ...
      thread_affinity[i] =
          (mpi_core < omp_size) ? omp_size + mpi_core : mpi_core - omp_size;
    }
#endif /* 0 */
  }
...

  {
    ...
    printf(
        ...
        a.hostname, hypre_rank, hypre_size, omp_rank, omp_size, comm_rank,
        comm_size, a.node_ID, a.core_ID);
    fflush(stdout);
  }
  ...
  if (omp_master_) { // only subcomm ranks must do this
    ...
    MPI_Comm_size(comm_omp_, &omp_size);
    printf(
        "Root on comm_omp_: comm_hypre_:%d/%d; comm_omp_:%d/%d; "
        "comm_:%d/%d\n",
        hypre_rank, hypre_size, omp_rank, omp_size, subrank, subsize);
  }
  MPI_Barrier(comm_hypre_);
  if (is_root) {
    fflush(stdout);
...
#pragma omp critical
      printf(
          "Thread %d/%d: host:%s; node_ID:%d; core_ID:%d\n", tid, nthreads,
          a.hostname, a.node_ID, a.core_ID);
    }
#else
...
#pragma omp for
    ...
    for (size_t i = 0; i < nthreads.size(); ++i) {
      ...
      printf(
          "omp_master %d/%d:\tnthreads:%d;\tspeedup:%4.2f;\t"
          "efficiency:%5.1f%%;\toverhead:%5.1f%%;\tresult=%.4e\n",
          comm_rank, comm_size, num_threads, t0i / ti,
          (t0i / ti) / num_threads * 100.0, (to - ti) / to * 100.0, result[i]);
    }
#endif /* _USE_OMP_ */
    command = 0;
    MPI_Bcast(&command, 1, MPI_INT, 0, comm_omp_);
