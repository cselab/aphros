...
static int alloc_ver(int nv, T* q) {
  MALLOC(nv, &q->hdg_ver);
  return HE_OK;
}

static int alloc_edg(int ne, T* q) {
  MALLOC(ne, &q->hdg_edg);
  return HE_OK;
}

static int alloc_tri(int nt, T* q) {
  MALLOC(nt, &q->hdg_tri);
  return HE_OK;
}
...
static void setup_edg(T* q, HeHash* hdg, int i, int j, int* pe) {
  ...
  q->edg[h] = e;
  if ((f = he_hash_get(hdg, j, i)) != -1) q->edg[f] = e;
...
#define NXT() \
  if (util_fgets(line, f) == NULL) ERR(("unexpected EOF in '%s'", path))
  MALLOC(1, &q);
  f = fopen(path, "r");
  if (f == NULL) ERR(("fail to open '%s'", path));
  NXT();
  if (!util_eq(line, "HE")) ERR(("'%s' is not a he file", path));
  NXT();
  cnt = sscanf(line, "%d %d %d %d", &nv, &ne, &nt, &nh);
  if (cnt != 4) ERR(("'%s' != [nv nt ne nh] in '%s'", line, path));
  ...
  for (i = 0; i < nh; i++) {
    NXT();
    cnt = sscanf(
        line, "%d %d  %d %d %d", &q->nxt[i], &q->flp[i], &q->ver[i], &q->edg[i],
        &q->tri[i]);
    if (cnt != 5) ERR(("wrong half-edg line '%s' in '%s'", line, path));
  }
  for (i = 0; i < nv; i++) {
    NXT();
    cnt = sscanf(line, "%d", &q->hdg_ver[i]);
    if (cnt != 1) ERR(("wrong ver line '%s' in '%s'", line, path));
  }
  for (i = 0; i < ne; i++) {
    NXT();
    cnt = sscanf(line, "%d", &q->hdg_edg[i]);
    if (cnt != 1) ERR(("wrong edg line '%s' in '%s'", line, path));
  }
  for (i = 0; i < nt; i++) {
    NXT();
    cnt = sscanf(line, "%d", &q->hdg_tri[i]);
    if (cnt != 1) ERR(("wrong tri line '%s' in '%s'", line, path));
  }
  if (fclose(f) != 0) ERR(("fail to close file '%s'", path));
  ...
  if (valid(nv, q->hdg_ver, q->ver) != OK) ERR(("invalid ver references"));
  if (valid(ne, q->hdg_edg, q->edg) != OK) ERR(("invalid edg references"));
  if (valid(nt, q->hdg_tri, q->tri) != OK) ERR(("invalid tri references"));
...
int he_read_fin(T* q) {
  if (q->magic != MAGIC) ERR(("wrong fin() call"));
  afree(q);
  FREE(q);
  return HE_OK;
}
...
int he_read_nxt(T* q, int** p) {
  *p = q->nxt;
  return HE_OK;
}

int he_read_flp(T* q, int** p) {
  *p = q->flp;
  return HE_OK;
}

int he_read_ver(T* q, int** p) {
  *p = q->ver;
  return HE_OK;
}

int he_read_tri(T* q, int** p) {
  *p = q->tri;
  return HE_OK;
}

int he_read_edg(T* q, int** p) {
  *p = q->edg;
  return HE_OK;
}

int he_read_hdg_ver(T* q, int** p) {
  *p = q->hdg_ver;
  return HE_OK;
}

int he_read_hdg_edg(T* q, int** p) {
  *p = q->hdg_edg;
  return HE_OK;
}

int he_read_hdg_tri(T* q, int** p) {
  *p = q->hdg_tri;
  return HE_OK;
}

static char* util_fgets(char* s, FILE* stream) {
  char* c;
