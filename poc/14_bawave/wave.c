// Created by Sergey Litvinov on 30.09.2019
...
double wave(double x, double y) {
  ...
  double eta2 =
      1. / 4. * alpa * (3. * sq(alpa) - 1.) * sq(a_) * k_ * cos(2. * k_ * x);
  double eta3 =
      -3. / 8. * (cube(alpa) * alpa - 3. * sq(alpa) + 3.) * cube(a_) * sq(k_) *
          cos(k_ * x) +
      3. / 64. *
          (8. * cube(alpa) * cube(alpa) + (sq(alpa) - 1.) * (sq(alpa) - 1.)) *
          cube(a_) * sq(k_) * cos(3. * k_ * x);
...
double gaus(double y, double yc, double T) {
  ...
  return 2.0 / (sqrt(2.0 * pi * sq(deltaa)) + sqrt(2.0 * pi * sq(deltaw))) *
         (T * exp(-sq(r) / (2.0 * sq(deltaw))) +
          (1.0 - T) * exp(-sq(r) / (2.0 * sq(deltaa))));
}
  ...
  if (!restore("restart")) {
    do {
      ...
      foreach () {
        ...
        double phi2 = 3. * ak * A_ / (8. * alpa) * (sq(alpa) - 1.) *
                      (sq(alpa) - 1.) * cosh(2.0 * k_ * (y + h_)) *
                      sin(2.0 * k_ * x) / cosh(2.0 * k_ * h_);
        double phi3 = 1. / 64. * (sq(alpa) - 1.) * (sq(alpa) + 3.) *
                      (9. * sq(alpa) - 13.) * cosh(3. * k_ * (y + h_)) /
                      cosh(3. * k_ * h_) * a_ * a_ * k_ * k_ * A_ *
                      sin(3. * k_ * x);
      ...
      if (DIRAC) {
        ...
        scalar psi[];
        foreach () {
          vort2[] = -2.0 * gaus(y, wave(x, y) + y, f[]) *
                    (Phi[1, 0] - Phi[-1, 0]) / (2. * Delta);
          psi[] = 0.0;
        }
        boundary({vort2, psi});
        psi[top] = dirichlet(0.);
        ...
        foreach ()
          foreach_dimension() u.x[] = (Phi[1] - Phi[-1]) / (2.0 * Delta) * f[];
      }
      boundary((scalar*){u});
    }
...
int dissipation_rate(double* rates) {
  ...
  foreach (reduction(+ : rateWater) reduction(+ : rateAir)) {
    ...
    double sqterm = 2. * dv() *
                    (sq(SDeformxx) + sq(SDeformxy) + sq(SDeformxz) +
...
event countDropsBubble(i++) {
  ...
  foreach_leaf() {
    ...
    if (m1[] > 0) {
      ...
      coord p = {x, y, z};
      foreach_dimension() b1[j].x += dv() * f[] * p.x;
    }
    // bubbles
    if (m2[] > 0) {
      int j = m2[] - 1;
      v2[j] += dv() * (1.0 - f[]);
      coord p = {x, y, z};
      foreach_dimension() b2[j].x += dv() * (1.0 - f[]) * p.x;
    }
  }
  ...
  for (int j = 0; j < n1; j++)
    fprintf(
        fdrop, "%d %g %d %g %g %g\n", i, t, j, v1[j], b1[j].x / v1[j],
        b1[j].y / v1[j]);
  for (int j = 0; j < n2; j++)
    fprintf(
        fbubb, "%d %g %d %g %g %g\n", i, t, j, v2[j], b2[j].x / v2[j],
        b2[j].y / v2[j]);
}
...
event graphs(i++) {
  ...
  foreach(reduction(+:ke) reduction(+:gpe) 
    ...
    double norm2 = 0.;
    foreach_dimension() norm2 += sq(u.x[]);
    ke += rho[] * norm2 * f[] * dv();
  ...
  fprintf(
      fpwater, "%g %g %g %g\n", t / (k_ / sqrt(g_ * k_)), ke / 2., gpe + 0.125,
      dissWater);
  fprintf(
      fpair, "%g %g %g %g\n", t / (k_ / sqrt(g_ * k_)), keAir / 2.,
      gpeAir + 0.125, dissAir);
  fprintf(
      ferr, "%g %g %g %g\n", t / (k_ / sqrt(g_ * k_)), ke / 2., gpe + 0.125,
      dissWater);
}
...
/**
~~~gnuplot Evolution of kinetic, potential and total energies and dissipation
rate. set key auto col
