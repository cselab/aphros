...
//-----------------------------------------------------------------------------
{
 ...
 public:
  CSG_Node()
      : op(None), prim(none), r(0), R(0), axe(X), left(NULL), right(NULL) {
    med[X] = med[Y] = med[Z] = min[X] = min[Y] = min[Z] = max[X] = max[Y] =
        max[Z] = 0;
  }
  ~CSG_Node() {
    delete left;
 ...
 public:
  ...
  const float eval(float x, float y, float z) const {
    ...
    switch (prim) {
      case sphere:
        res =
            ((x - med[X]) * (x - med[X]) + (y - med[Y]) * (y - med[Y]) +
             (z - med[Z]) * (z - med[Z]) - r * r);
        break;
      ...
      case heart:
        res = (2 * x * x + y * y + z * z - 1) *
                  (2 * x * x + y * y + z * z - 1) *
                  (2 * x * x + y * y + z * z - 1) -
              (1 / 10) * x * x * z * z * z - y * y * z * z * z;
        break;

      case tangle:
        res = x * x * x * x - 5 * x * x + y * y * y * y - 5 * y * y +
              z * z * z * z - 5 * z * z + 11.8f;
        break;
      ...
      case torus:
        ...
        t = sqrt(
            i * i - 2 * i * med[X] + med[X] * med[X] + k * k - 2 * k * med[Z] +
            med[Z] * med[Z]);
        res =
            (t * (med[X] * med[X] - 2 * i * med[X] + med[Z] * med[Z] + k * k +
                  med[Y] * med[Y] - 2 * k * med[Z] + j * j - 2 * j * med[Y] +
                  r * r + i * i) +
             (-2 * r * med[X] * med[X] + 4 * r * i * med[X] +
              4 * r * k * med[Z] - 2 * r * med[Z] * med[Z] - 2 * r * i * i -
              2 * r * k * k) -
             t * R * R);
        break;
      ...
      case cone:
        if (z > max[Z] || z < min[Z])
          ...
          res =
              ((float)hypot(x, y) -
               (r + (z - min[Z]) / (max[Z] - min[Z]) * (R - r)));
        break;

      case block:
        if (x < min[X] || y < min[Y] || z < min[Z] || x > max[X] ||
            y > max[Y] || z > max[Z])
          res = 1;
        else
          res = -1;
