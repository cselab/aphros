...
//-----------------------------------------------------------------------------
{
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (!strcmp(argv[i], "-iply")) {
      ...
      if (++i != argc) {
        size_x = size_y = size_z = atoi(argv[i]);
      }
      ...
      printf(
          "usage %s [-i file.iso] [-iply file.ply] [-c file.csg] [-f "
          "'formula'] [-r[xyz] res] -R [-iv file.iv] [-ply file.ply] [-ppm "
          "file.ppm] [-eps file.eps] [-q]\n",
          argv[0]);
    }
  }
  glui_side->sync_live();
