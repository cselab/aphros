...
static int gl2psDeflate(void) {
  ...
  return compress(
      gl2ps->compress->dest, &gl2ps->compress->destLen, gl2ps->compress->start,
...
static void gl2psFillTriangleFromPrimitive(
    GL2PStriangle* t, GL2PSprimitive* p, GLboolean assignprops) {
...
static void gl2psListSort(
    GL2PSlist* list, int (*fcmp)(const void* a, const void* b)) {
  if (!list) return;
...
static void gl2psListActionInverse(
    GL2PSlist* list, void (*action)(void* data)) {
  GLint i;
...
static void gl2psCutEdge(
    GL2PSvertex* a, GL2PSvertex* b, GL2PSplane plane, GL2PSvertex* c) {
  GL2PSxyz v;
  GLfloat sect;
...
static void gl2psCreateSplitPrimitive(
    GL2PSprimitive* parent, GL2PSplane plane, GL2PSprimitive* child,
    GLshort numverts, GLshort* index0, GLshort* index1) {
  GLshort i;
  ...
  for (i = 0; i < numverts; i++) {
    if (index1[i] < 0) {
      ...
      gl2psCutEdge(
          &parent->verts[index0[i]], &parent->verts[index1[i]], plane,
          &child->verts[i]);
    }
  }
}

static void gl2psAddIndex(
    GLshort* index0, GLshort* index1, GLshort* nb, GLshort i, GLshort j) {
  GLint k;
...
static GLint gl2psSplitPrimitive(
    GL2PSprimitive* prim, GL2PSplane plane, GL2PSprimitive** front,
    GL2PSprimitive** back) {
...
static void gl2psDivideQuad(
    GL2PSprimitive* quad, GL2PSprimitive** t1, GL2PSprimitive** t2) {
...
static void gl2psTraverseBspTree(
    GL2PSbsptree* tree, GL2PSxyz eye, GLfloat epsilon,
    GLboolean (*compare)(GLfloat f1, GLfloat f2), void (*action)(void* data),
    int inverse) {
  GLfloat result;
...
static void gl2psAddPlanesInBspTreeImage(
    GL2PSprimitive* prim, GL2PSbsptree2d** tree) {
  GLint ret = 0;
  GLint i;
  GLint offset = 0;
  ...
  if ((*tree == NULL) && (prim->numverts > 2)) {
    ...
    for (i = 0; i < prim->numverts - 1; i++) {
      if (!gl2psGetPlaneFromPoints(
              prim->verts[i].xyz, prim->verts[i + 1].xyz, head->plane)) {
    ...
    switch (ret) {
      case GL2PS_POINT_INFRONT:
        ...
        for (i = 1 + offset; i < prim->numverts - 1; i++) {
          ...
          if (gl2psGetPlaneFromPoints(
                  prim->verts[i].xyz, prim->verts[i + 1].xyz,
                  cur->front->plane)) {
            cur = cur->front;
            cur->front = NULL;
            cur->back = NULL;
        ...
        if (gl2psGetPlaneFromPoints(
                prim->verts[i].xyz, prim->verts[offset].xyz,
                cur->front->plane)) {
      ...
      case GL2PS_POINT_BACK:
        ...
        for (i = 1 + offset; i < prim->numverts - 1; i++) {
          ...
          if (gl2psGetPlaneFromPoints(
                  prim->verts[i + 1].xyz, prim->verts[i].xyz,
                  cur->front->plane)) {
            cur = cur->front;
            cur->front = NULL;
            cur->back = NULL;
        ...
        if (gl2psGetPlaneFromPoints(
                prim->verts[offset].xyz, prim->verts[i].xyz,
                cur->front->plane)) {
          cur->front->front = NULL;
          cur->front->back = NULL;
        } else {
...
static GL2PSprimitive* gl2psCreateSplitPrimitive2D(
    GL2PSprimitive* parent, GLshort numverts, GL2PSvertex* vertx) {
  GLint i;
...
static void gl2psSplitPrimitive2D(
    GL2PSprimitive* prim, GL2PSplane plane, GL2PSprimitive** front,
    GL2PSprimitive** back) {
  ...
  for (i = 0; i <= prim->numverts; i++) {
    ...
    if (((prev == -1) || (prev == cur) || (prev == 0) || (cur == 0)) &&
      ...
      front_list = (GL2PSvertex*)gl2psRealloc(
          front_list, sizeof(GL2PSvertex) * front_count);
      gl2psCutEdge(
          &prim->verts[v2], &prim->verts[v1], plane,
...
static GLint gl2psAddInBspImageTree(
    GL2PSprimitive* prim, GL2PSbsptree2d** tree) {
  GLint ret = 0;
...
static void gl2psAddPolyPrimitive(
    GLshort type, GLshort numverts, GL2PSvertex* verts, GLint offset,
    ...
    switch ((GLint)*current) {
      case GL_POINT_TOKEN:
        ...
        gl2psAddPolyPrimitive(
            GL2PS_POINT, 1, vertices, 0, pattern, factor, psize, 0);
        break;
      case GL_LINE_TOKEN:
      case GL_LINE_RESET_TOKEN:
        ...
        used -= i;
        gl2psAddPolyPrimitive(
            GL2PS_LINE, 2, vertices, 0, pattern, factor, lwidth, 0);
        break;
        ...
        while (count > 0 && used > 0) {
          ...
          if (v == 2) {
            if (GL_TRUE == boundary) {
              ...
              flag = 0;
            gl2psAddPolyPrimitive(
                GL2PS_TRIANGLE, 3, vertices, offset, pattern, factor, 1, flag);
      ...
      case GL_PASS_THROUGH_TOKEN:
        switch ((GLint)current[1]) {
          ...
          case GL2PS_END_LINE_STIPPLE:
            pattern = factor = 0;
            break;
          case GL2PS_BEGIN_BLEND:
            gl2ps->blending = GL_TRUE;
            break;
          case GL2PS_END_BLEND:
          ...
          case GL2PS_DRAW_IMAGEMAP_TOKEN:
            ...
            for (i = 0; i < vtot; i += sizeoffloat) {
              ...
              if ((vtot - i) >= 4)
                memcpy(
                    &(((char*)(node->image->pixels))[i + v]), &(current[2]),
                    sizeoffloat);
              else
                memcpy(
                    &(((char*)(node->image->pixels))[i + v]), &(current[2]),
...
static GLfloat gl2psGetRGB(
    GL2PSimage* im, GLuint x, GLuint y, GLfloat* red, GLfloat* green,
    GLfloat* blue) {
...
static void gl2psPrintPostScriptImagemap(
    GLfloat x, GLfloat y, GLsizei width, GLsizei height,
...
static void gl2psPrintPostScriptHeader(void) {
  ...
  gl2psPrintf(
      "%%%%Title: %s\n"
      "%%%%Creator: GL2PS %d.%d.%d, (C) 1999-2004 Christophe Geuzaine "
      "<geuz@geuz.org>\n"
      "%%%%For: %s\n"
      "%%%%CreationDate: %s"
      "%%%%LanguageLevel: 3\n"
      ...
      GL2PS_PATCH_VERSION, gl2ps->producer, ctime(&now));
  ...
  if (gl2ps->options & GL2PS_DRAW_BACKGROUND) {
    ...
    gl2psPrintf(
        ...
        "closepath fill\n",
        rgba[0], rgba[1], rgba[2], (int)gl2ps->viewport[0],
        (int)gl2ps->viewport[1], (int)gl2ps->viewport[2],
        (int)gl2ps->viewport[1], (int)gl2ps->viewport[2],
        (int)gl2ps->viewport[3], (int)gl2ps->viewport[0],
        (int)gl2ps->viewport[3]);
  }
}
...
static int gl2psPrintDash(GLushort pattern, GLint factor, const char* str) {
  ...
  switch (prim->type) {
    case GL2PS_PIXMAP:
      gl2psPrintPostScriptPixmap(
          prim->verts[0].xyz[0], prim->verts[0].xyz[1], prim->data.image);
      break;
    case GL2PS_IMAGEMAP:
    ...
    case GL2PS_TEXT:
      gl2psPrintPostScriptColor(prim->verts[0].rgba);
      gl2psPrintf(
          "(%s) %g %g %d /%s ", prim->data.text->str, prim->verts[0].xyz[0],
    ...
    case GL2PS_TRIANGLE:
      if (!gl2psVertsSameColor(prim)) {
        ...
        gl2psPrintf(
            "%g %g %g %g %g %g T\n", prim->verts[2].xyz[0],
            prim->verts[2].xyz[1], prim->verts[1].xyz[0], prim->verts[1].xyz[1],
            prim->verts[0].xyz[0], prim->verts[0].xyz[1]);
      }
      break;
    case GL2PS_QUADRANGLE:
...
static void gl2psPrintPostScriptFooter(void) {
  ...
  if (gl2ps->options & GL2PS_COMPRESS) {
    if (Z_OK != gl2psDeflate()) {
      ...
      fwrite(
          gl2ps->compress->dest + n, gl2ps->compress->destLen - (n + 4), 1,
          gl2ps->stream);
...
static void gl2psPrintTeXPrimitive(void* data) {
  ...
  switch (prim->type) {
    case GL2PS_TEXT:
      fprintf(
          gl2ps->stream, "\\fontsize{%d}{0}\n\\selectfont",
          prim->data.text->fontsize);
      fprintf(
          gl2ps->stream, "\\put(%g,%g){\\makebox(0,0)", prim->verts[0].xyz[0],
          prim->verts[0].xyz[1]);
      switch (prim->data.text->alignment) {
        case GL2PS_TEXT_CL:
      ...
      fprintf(
          gl2ps->stream, "\\textcolor[rgb]{%g,%g,%g}{{%s}}",
          prim->verts[0].rgba[0], prim->verts[0].rgba[1],
    ...
    switch (prim->type) {
      case GL2PS_PIXMAP:
        for (j = 0; j <= lastel; ++j) {
          ...
          gl2psPutPDFImage(
              prim->data.image, gro->imno, prim->verts[0].xyz[0],
              prim->verts[0].xyz[1]);
        }
        break;
      case GL2PS_TEXT:
        for (j = 0; j <= lastel; ++j) {
          ...
          gl2psPutPDFText(
              prim->data.text, gro->fontno, prim->verts[0].xyz[0],
              prim->verts[0].xyz[1]);
        }
        break;
      case GL2PS_LINE:
...
static int gl2psPrintPDFShaderStreamDataRGB(
    GL2PSvertex* vertex, size_t (*action)(unsigned long data, size_t size)) {
  int offs = 0;
  unsigned long imap;
  double dmax = ~1UL;
...
static int gl2psPrintPDFShaderStreamData(
  ...
  for (i = 0; i < 3; ++i) {
    offs += gl2psPrintPDFShaderStreamDataCoord(
        &triangle->vertex[i], action, dx, dy, xmin, ymin);
    if (gray) {
      v = triangle->vertex[i];
...
static int gl2psPrintPDFShader(
    int obj, GL2PStriangle* triangles, int size, int gray) {
  ...
  offs += fprintf(
      ...
      obj, (gray) ? "/DeviceGray" : "/DeviceRGB", (gray) ? gray : 8, xmin, xmax,
      ymin, ymax, (gray) ? "" : "0 1 0 1");
  ...
  if (gl2ps->options & GL2PS_COMPRESS) {
    ...
    for (i = 0; i < size; ++i)
      gl2psPrintPDFShaderStreamData(
          &triangles[i], xmax - xmin, ymax - ymin, xmin, ymin,
    ...
    if (Z_OK == gl2psDeflate() &&
      ...
      offs += gl2ps->compress->destLen * fwrite(
                                             gl2ps->compress->dest,
...
static int gl2psPrintPDFPixmapStreamData(
    GL2PSimage* im, size_t (*action)(unsigned long data, size_t size),
    int gray) {
  int x, y;
  GLfloat r, g, b, a;
...
static int gl2psPrintPDFPixmap(
    int obj, int childobj, GL2PSimage* im, int gray) {
  ...
  for (i = 0; i < gl2psListNbr(gl2ps->pdfgrouplist); ++i) {
    ...
    switch (p->type) {
      case GL2PS_PIXMAP:
        ...
        offs += gl2psPrintPDFPixmap(
            gro->imobjno, gro->imobjno + 1, p->data.image, 0);
...
static void gl2psPrintIPEHeader(void) {
  ...
  fprintf(
      ...
      gl2ps->title, gl2ps->producer, gl2ps->viewport[0], gl2ps->viewport[1],
  ...
  if (gl2ps->options & GL2PS_DRAW_BACKGROUND) {
    ...
    fprintf(
        ...
        rgba[0], rgba[1], rgba[2], gl2ps->viewport[0], gl2ps->viewport[1],
        gl2ps->viewport[2], gl2ps->viewport[1], gl2ps->viewport[2],
        gl2ps->viewport[3], gl2ps->viewport[0], gl2ps->viewport[3]);
  }
}
...
static void gl2psPrintIPEPrimitive(void* data) {
  ...
  switch (prim->type) {
    ...
    case GL2PS_TEXT:
      ...
      gl2psPrintf(
          "<text size=\"%d\" stroke=\"%g %g %g\" pos=\"%g %g\">",
          prim->data.text->fontsize, prim->verts[0].rgba[0],
          prim->verts[0].rgba[1], prim->verts[0].rgba[2], prim->verts[0].xyz[0],
          prim->verts[0].xyz[1]);
      gl2psPrintf(
          "\\fontsize{%d}{0}\\selectfont{\\makebox(0,0)",
          prim->data.text->fontsize);
      switch (prim->data.text->alignment) {
        case GL2PS_TEXT_CL:
          gl2psPrintf("[l]{");
      ...
      gl2psPrintf("\\textcolor[rgb]{%g,%g,%g}{{%s}}", prim->data.text->str);
      if (prim->data.text->angle) gl2psPrintf("}");
      gl2psPrintf("}}</text>");
      break;
    case GL2PS_POINT:
      gl2psSetLastColor(prim->verts[0].rgba);
      gl2psPrintf(
          " <mark stroke=\"%g %g %g\" shape=\"2\" pos=\"%g %g\" "
          "size=\"%g\"/>\n",
    ...
    case GL2PS_LINE:
      ...
      gl2psPrintf(
          "<path stroke=\"%g %g %g\" dash=\"%s\" pen=\"%g\"> %g %g m %g %g l "
          "</path>\n",
          gl2ps->lastrgba[0], gl2ps->lastrgba[1], gl2ps->lastrgba[2],
    ...
    case GL2PS_TRIANGLE:
      gl2psSetLastColor(prim->verts[0].rgba);
      gl2psPrintf(
          "<path fill=\"%g %g %g\"> %g %g m %g %g l %g %g l h </path>\n",
          gl2ps->lastrgba[0], gl2ps->lastrgba[1], gl2ps->lastrgba[2],
...
static void gl2psPrintRAWPrimitive(void* data) {
  ...
  switch (prim->type) {
    ...
    case GL2PS_TRIANGLE:
      gl2psPrintf(
          "%g %g %g %g %g %g %g %g %g\n", prim->verts[2].xyz[0],
          prim->verts[2].xyz[1], prim->verts[2].xyz[2], prim->verts[1].xyz[0],
          prim->verts[1].xyz[1], prim->verts[1].xyz[2], prim->verts[0].xyz[0],
          prim->verts[0].xyz[1], prim->verts[0].xyz[2]);
      break;
    case GL2PS_QUADRANGLE:
      gl2psMsg(GL2PS_WARNING, "There should not be any quad left to print");
      break;
...
static GLint gl2psPrintPrimitives(void) {
  ...
  if (used > 0) {
    if (gl2ps->format == GL2PS_PS || gl2ps->format == GL2PS_EPS ||
        gl2ps->format == GL2PS_PDF || gl2ps->format == GL2PS_IPE ||
    ...
    case GL2PS_BSP_SORT:
      ...
      if (gl2ps->options & GL2PS_OCCLUSION_CULL) {
        gl2psTraverseBspTree(
            root, eye, -GL2PS_EPSILON, gl2psLess, gl2psAddInImageTree, 1);
        gl2psFreeBspImageTree(&gl2ps->imagetree);
      }
      gl2psTraverseBspTree(root, eye, GL2PS_EPSILON, gl2psGreater, pprim, 0);
...
GLint gl2psBeginPage(
  ...
  if (!gl2ps->viewport[2] || !gl2ps->viewport[3]) {
    gl2psMsg(
        GL2PS_ERROR, "Incorrect viewport (x=%d, y=%d, width=%d, height=%d)",
...
GLint gl2psTextOpt(
    const char* str, const char* fontname, GLshort fontsize, GLint alignment,
    GLfloat angle) {
  GLfloat pos[4];
  GL2PSprimitive* prim;
  GLboolean valid;
