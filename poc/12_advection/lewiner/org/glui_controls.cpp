...
//-----------------------------------------------------------------------------
{
  ...
  switch (control) {
    ...
    case CASE_ID:
      sprintf(
          formula,
          "(%f)*(1-x)*(1-y)*(1-z)+(%f)*(1+x)*(1-y)*(1-z)+(%f)*(1-x)*(1+y)*(1-z)"
          "+(%f)*(1+x)*(1+y)*(1-z)+(%f)*(1-x)*(1-y)*(1+z)+(%f)*(1+x)*(1-y)*(1+"
          "z)+(%f)*(1-x)*(1+y)*(1+z)+(%f)*(1+x)*(1+y)*(1+z)",
    ...
    case LOAD_VIEWPORT_ID:
      ...
      if (!fp) break;
      fscanf(
          fp, "rotate: %f %f %f %f  %f %f %f %f  %f %f %f %f  %f %f %f %f ",
          view_rotate + 0, view_rotate + 1, view_rotate + 2, view_rotate + 3,
          view_rotate + 4, view_rotate + 5, view_rotate + 6, view_rotate + 7,
          view_rotate + 8, view_rotate + 9, view_rotate + 10, view_rotate + 11,
          view_rotate + 12, view_rotate + 13, view_rotate + 14,
          view_rotate + 15);
    ...
    case RESET_ROTATION_ID:
      view_rotate[0] = view_rotate[5] = view_rotate[10] = view_rotate[15] =
          1.0f;
      view_rotate[1] = view_rotate[2] = view_rotate[3] = view_rotate[4] = 0.0f;
      view_rotate[6] = view_rotate[7] = view_rotate[8] = view_rotate[9] = 0.0f;
      view_rotate[11] = view_rotate[12] = view_rotate[13] = view_rotate[14] =
          0.0f;
      break;
    // reset translation
    case RESET_TRANSLATION_ID:
      obj_pos[0] = obj_pos[1] = 0.0f;
      break;
    // reset zoom
    case RESET_ZOOM_ID:
      obj_pos[2] = 0.0f;
...
//-----------------------------------------------------------------------------
{
  ...
  glui_side =
      GLUI_Master.create_glui_subwindow(main_window, GLUI_SUBWINDOW_RIGHT);
  ...
  // Input : functions
  list = glui_side->add_listbox_to_panel(
      roll, "Implicit Functions:", &curr_string, FUN_ID, control_cb);
  for (int i = 0; i < NFUNS; i++)
    list->add_item(i, fun_list[i]);

  // Input : trilinear function
  roll2 = glui_side->add_rollout_to_panel(roll, "MC Case", false);
  text =
      glui_side->add_edittext_to_panel(roll2, "v0", GLUI_EDITTEXT_FLOAT, v + 0);
  text->set_w(4);
  text =
      glui_side->add_edittext_to_panel(roll2, "v1", GLUI_EDITTEXT_FLOAT, v + 1);
  text->set_w(4);
  text =
      glui_side->add_edittext_to_panel(roll2, "v2", GLUI_EDITTEXT_FLOAT, v + 2);
  text->set_w(4);
  text =
      glui_side->add_edittext_to_panel(roll2, "v3", GLUI_EDITTEXT_FLOAT, v + 3);
  text->set_w(4);
  text =
      glui_side->add_edittext_to_panel(roll2, "v4", GLUI_EDITTEXT_FLOAT, v + 4);
  text->set_w(4);
  text =
      glui_side->add_edittext_to_panel(roll2, "v5", GLUI_EDITTEXT_FLOAT, v + 5);
  text->set_w(4);
  text =
      glui_side->add_edittext_to_panel(roll2, "v6", GLUI_EDITTEXT_FLOAT, v + 6);
  text->set_w(4);
  text =
      glui_side->add_edittext_to_panel(roll2, "v7", GLUI_EDITTEXT_FLOAT, v + 7);
  text->set_w(4);
  glui_side->add_button_to_panel(roll2, "Set Formula", CASE_ID, control_cb);
  /*
    glui_side->add_statictext_to_panel( roll, "" );
    text = glui_side->add_edittext_to_panel( roll, "v4", GLUI_EDITTEXT_FLOAT,
    &v4, CASE_ID, control_cb ) ; text->set_w(4) ;
    glui_side->add_statictext_to_panel( roll, "" );
    glui_side->add_statictext_to_panel( roll, "" );
    text = glui_side->add_edittext_to_panel( roll, "v0", GLUI_EDITTEXT_FLOAT,
    &v0, CASE_ID, control_cb ) ; text->set_w(4) ;
    glui_side->add_column_to_panel( roll, false ) ;
    text = glui_side->add_edittext_to_panel( roll, "v7", GLUI_EDITTEXT_FLOAT,
    &v7, CASE_ID, control_cb ) ; text->set_w(4) ;
    glui_side->add_statictext_to_panel( roll, "" );
    glui_side->add_statictext_to_panel( roll, "" );
    text = glui_side->add_edittext_to_panel( roll, "v3", GLUI_EDITTEXT_FLOAT,
    &v3, CASE_ID, control_cb ) ; text->set_w(4) ;
    glui_side->add_statictext_to_panel( roll, "" );
    glui_side->add_column_to_panel( roll, false ) ;
    glui_side->add_statictext_to_panel( roll, "" );
    text = glui_side->add_edittext_to_panel( roll, "v5", GLUI_EDITTEXT_FLOAT,
    &v5, CASE_ID, control_cb ) ; text->set_w(4) ;
    glui_side->add_statictext_to_panel( roll, "" );
    glui_side->add_statictext_to_panel( roll, "" );
    text = glui_side->add_edittext_to_panel( roll, "v1", GLUI_EDITTEXT_FLOAT,
    &v1, CASE_ID, control_cb ) ; text->set_w(4) ;
    glui_side->add_column_to_panel( roll, false ) ;
    text = glui_side->add_edittext_to_panel( roll, "v6", GLUI_EDITTEXT_FLOAT,
    &v6, CASE_ID, control_cb ) ; text->set_w(4) ;
    glui_side->add_statictext_to_panel( roll, "" );
    glui_side->add_statictext_to_panel( roll, "" );
    text = glui_side->add_edittext_to_panel( roll, "v2", GLUI_EDITTEXT_FLOAT,
    &v2, CASE_ID, control_cb ) ; text->set_w(4) ;
    glui_side->add_statictext_to_panel( roll, "" );
  */
  ...
  open_csg_ctrl =
      glui_side->add_button_to_panel(roll, "Open CSG", CSG_ID, control_cb);
  open_iso_ctrl =
      glui_side->add_button_to_panel(roll, "Open ISO", ISO_ID, control_cb);
  ...
  xres_ctrl =
      glui_side->add_edittext_to_panel(roll, "X", GLUI_EDITTEXT_INT, &size_x);
  yres_ctrl =
      glui_side->add_edittext_to_panel(roll, "Y", GLUI_EDITTEXT_INT, &size_y);
  zres_ctrl =
      glui_side->add_edittext_to_panel(roll, "Z", GLUI_EDITTEXT_INT, &size_z);

  //--------------------------------------------------//
  // Run
  roll = glui_side->add_rollout("Run", true);
  formula_ctrl = glui_side->add_edittext_to_panel(
      roll, "Formula", GLUI_EDITTEXT_TEXT, formula);
  ...
  out_filename =
      glui_side->add_edittext_to_panel(roll, "File name", GLUI_EDITTEXT_TEXT);
  out_filename->set_w(200);
  glui_side->add_checkbox_to_panel(
      roll, "Export ISO at next MC run", &export_iso);
  pan = glui_side->add_panel_to_panel(roll, "", GLUI_PANEL_NONE);
...
//-----------------------------------------------------------------------------
{
  ...
  objects_mv =
      glui_bottom->add_translation("Objects XY", GLUI_TRANSLATION_XY, obj_pos);
  objects_mv->set_speed(.005f);
  glui_bottom->add_button("Reset", RESET_TRANSLATION_ID, control_cb);
  glui_bottom->add_column(false);

  objects_zm = glui_bottom->add_translation(
      "Objects Z", GLUI_TRANSLATION_Z, &obj_pos[2]);
  ...
  rot->set_spin(.82f);
  glui_bottom->add_checkbox(
      "On", &light0_enabled, LIGHT0_ENABLED_ID, control_cb);
  light0_ctrl = glui_bottom->add_edittext(
      "I (%)", GLUI_EDITTEXT_INT, &light0_intensity, LIGHT0_INTENSITY_ID,
      control_cb);
  ...
  rot->set_spin(.82f);
  glui_bottom->add_checkbox(
      "On", &light1_enabled, LIGHT1_ENABLED_ID, control_cb);
  light1_ctrl = glui_bottom->add_edittext(
      "I (%)", GLUI_EDITTEXT_INT, &light1_intensity, LIGHT0_INTENSITY_ID,
      control_cb);
