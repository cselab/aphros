...
//-----------------------------------------------------------------------------
{
  ...
  gl2psBeginPage(
      out_filename->get_text(),
      "Topological Marching Cubes (by Thomas Lewiner)", viewport_gl, GL2PS_EPS,
      GL2PS_SIMPLE_SORT, // GL2PS_BSP_SORT,
      GL2PS_SIMPLE_LINE_OFFSET | GL2PS_NO_PS3_SHADING | GL2PS_OCCLUSION_CULL,
      GL_RGBA, 0, NULL, 0, 0, 0, 100 * viewport[2] * viewport[3], fp, NULL);
  display();
  gl2psEndPage();
...
//-----------------------------------------------------------------------------
{
  ...
  fprintf(fp, "P6\n%d %d\n", tw, th);
  fprintf(
      fp, "# %s (Topological Marching Cubes by Thomas Lewiner)\n",
      (const char*)out_filename);
  fprintf(fp, "255\n");
