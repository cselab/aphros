...
PlyFile* ply_write(
    FILE* fp, int nelems, const char** elem_names, int file_type) {
  int i;
  PlyFile* plyfile;
  PlyElement* elem;
...
PlyFile* open_for_writing_ply(
    char* filename, int nelems, const char** elem_names, int file_type) {
  PlyFile* plyfile;
  char* name;
  FILE* fp;
...
void ply_describe_property(
    PlyFile* plyfile, char* elem_name, PlyProperty* prop) {
  PlyElement* elem;
  ...
  if (elem->nprops == 0) {
    ...
    elem->props = (PlyProperty**)realloc(
        elem->props, sizeof(PlyProperty*) * elem->nprops);
    elem->store_prop =
...
void header_complete_ply(PlyFile* plyfile) {
  ...
  switch (plyfile->file_type) {
    ...
    default:
      fprintf(
          stderr, "ply_header_complete: bad file type = %d\n",
          plyfile->file_type);
      exit(-1);
  }
...
void put_element_setup_ply(PlyFile* plyfile, const char* elem_name) {
  ...
  if (elem == NULL) {
    fprintf(
        stderr, "put_element_setup_ply: can't find element '%s'\n", elem_name);
    exit(-1);
  }
  ...
  if (plyfile->file_type == PLY_ASCII) {
    ...
    for (j = 0; j < elem->nprops; j++) {
      ...
      if (prop->is_list == PLY_LIST) {
        ...
        item = elem_data + prop->offset;
        get_stored_item(
            (void*)item, prop->internal_type, &int_val, &uint_val, &double_val);
        write_ascii_item(
            fp, int_val, uint_val, double_val, prop->external_type);
      }
    }
...
PlyFile* ply_open_for_reading(
    char* filename, int* nelems, char*** elem_names, int* file_type,
    float* version) {
  FILE* fp;
  PlyFile* plyfile;
  char* name;
...
void get_element_setup_ply(
    PlyFile* plyfile, char* elem_name, int nprops, PlyProperty* prop_list) {
  int i;
  PlyElement* elem;
  PlyProperty* prop;
  int index;
...
static PlyOtherProp* get_other_properties(
    PlyFile* plyfile, PlyElement* elem, int offset) {
  int i;
  PlyOtherProp* other;
  PlyProperty* prop;
  int nprops;
...
PlyOtherProp* ply_get_other_properties(
    PlyFile* plyfile, char* elem_name, int offset) {
  PlyElement* elem;
  PlyOtherProp* other;
  ...
  if (elem == NULL) {
    fprintf(
        stderr, "ply_get_other_properties: Can't find element '%s'\n",
        elem_name);
    return (NULL);
  }
...
PlyOtherElems* get_other_element_ply(PlyFile* plyfile) {
  ...
  if (plyfile->other_elems == NULL) {
    ...
    other_elems = plyfile->other_elems;
    other_elems->other_list = (OtherElem*)realloc(
...
PlyProperty* find_property(
    PlyElement* elem, const char* prop_name, int* index) {
  int i;
...
void ascii_get_element(PlyFile* plyfile, char* elem_ptr) {
  ...
  for (j = 0; j < elem->nprops; j++) {
    ...
    if (prop->is_list == PLY_LIST) {
      ...
      get_ascii_item(
          words[which_word++], prop->count_external, &int_val, &uint_val,
          &double_val);
      if (store_it) {
        item = elem_data + prop->count_offset;
      ...
      if (list_count == 0) {
        ...
        for (k = 0; k < list_count; k++) {
          get_ascii_item(
              words[which_word++], prop->external_type, &int_val, &uint_val,
              &double_val);
          if (store_it) {
            store_item(
...
void binary_get_element(PlyFile* plyfile, char* elem_ptr) {
  ...
  for (j = 0; j < elem->nprops; j++) {
    ...
    if (prop->is_list == PLY_LIST) {
      ...
      /* get and store the number of items in the list */
      get_binary_item(
          fp, prop->count_external, &int_val, &uint_val, &double_val);
      if (store_it) {
        item = elem_data + prop->count_offset;
        store_item(item, prop->count_internal, int_val, uint_val, double_val);
      }
      ...
      if (list_count == 0) {
        ...
        for (k = 0; k < list_count; k++) {
          get_binary_item(
              fp, prop->external_type, &int_val, &uint_val, &double_val);
          if (store_it) {
            store_item(
                item, prop->internal_type, int_val, uint_val, double_val);
            item += item_size;
...
void get_binary_item(
    FILE* fp, int type, int* int_val, unsigned int* uint_val,
    double* double_val) {
  char c[8];
  void* ptr;
...
void get_ascii_item(
    char* word, int type, int* int_val, unsigned int* uint_val,
    double* double_val) {
  switch (type) {
    case Int8:
    case Uint8:
...
void store_item(
    char* item, int type, int int_val, unsigned int uint_val,
    double double_val) {
  unsigned char* puchar;
  short int* pshort;
  unsigned short int* pushort;
...
int get_prop_type(char* type_name) {
  ...
  for (i = StartType + 1; i < EndType; i++)
    if (equal_strings(type_name, old_type_names[i])) return (i);
...
void append_comment_ply(PlyFile* ply, char* comment) {
  ...
  if (ply->num_comments == 0)
    ...
    ply->comments =
        (char**)realloc(ply->comments, sizeof(char*) * (ply->num_comments + 1));
...
void append_obj_info_ply(PlyFile* ply, char* obj_info) {
  ...
  if (ply->num_obj_info == 0)
    ...
    ply->obj_info =
        (char**)realloc(ply->obj_info, sizeof(char*) * (ply->num_obj_info + 1));
...
void describe_element_ply(PlyFile* plyfile, const char* elem_name, int nelems) {
  ...
  if (elem == NULL) {
    fprintf(
        stderr, "describe_element_ply: can't find element '%s'\n", elem_name);
    exit(-1);
  }
...
void describe_other_properties_ply(
  ...
  if (elem == NULL) {
    fprintf(
        stderr, "describe_other_properties_ply: can't find element '%s'\n",
        other->name);
    return;
  }
...
PlyPropRules* init_rule_ply(PlyFile* ply, char* elem_name) {
  ...
  for (list = ply->rule_list; list != NULL; list = list->next) {
    ...
    if (!found_prop) {
      fprintf(
          stderr, "Can't find property '%s' for rule '%s'\n", list->property,
          list->name);
      continue;
    }
  }
    ...
    switch (rules->rule_list[i]) {
      ...
      case MINIMUM_RULE: {
        ...
        for (j = 1; j < rules->nprops; j++)
          if (double_val > vals[j]) double_val = vals[j];
        break;
      }
      case MAXIMUM_RULE: {
        double_val = vals[0];
      ...
      case SAME_RULE: {
        ...
        for (j = 1; j < rules->nprops; j++)
          if (double_val != vals[j]) {
            fprintf(
                stderr,
                "get_new_props_ply: Error combining properties that should be "
                "the same.\n");
            exit(-1);
          }
        break;
      ...
      default:
        fprintf(
            stderr, "get_new_props_ply: Bad rule = %d\n", rules->rule_list[i]);
        exit(-1);
    }
...
PlyRuleList* append_prop_rule(
    PlyRuleList* rule_list, char* name, char* property) {
  PlyRuleList* rule;
...
int matches_rule_name(char* name) {
  ...
  for (i = 0; rule_name_list[i].code != -1; i++)
    if (equal_strings(rule_name_list[i].name, name)) return (1);
