...
enum { X, Y, Z };
enum { OOO, IOO, IIO, OIO, OOI, IOI, III, OII };
...
static int COff[][4] = {
    {0, 0, 0, X}, {1, 0, 0, Y}, {0, 1, 0, X}, {0, 0, 0, Y},
    {0, 0, 1, X}, {1, 0, 1, Y}, {0, 1, 1, X}, {0, 0, 1, Y},
    {0, 0, 0, Z}, {1, 0, 0, Z}, {1, 1, 0, Z}, {0, 1, 0, Z},
};
...
struct MarchLewiner* march_lewiner_ini(int x0, int y0, int z0) {
  int i;
  int N;
...
int march_lewiner_fin(struct MarchLewiner* q) {
  free(q->verts);
  free(q->ver);
  free(q->tri);
  free(q);
...
int march_lewiner_apply(
    struct MarchLewiner* q0, double* data, int* nv, double** ver, int* nt,
    int** tri) {
  int status;
...
static int get_vert(int D, int i, int j, int k) {
  int x, y;
...
static int set_vert(int D, int i, int j, int k) {
  double u;
  int d;
  int x, y;
  double* cu;
    ...
    for (j = 0; j < y; j++)
      for (i = 0; i < x; i++) {
        ...
        if (k < z - 1)
          ...
          cu[OOI] = cu[OOO];
        if (cu[OOO] < 0) {
          if (cu[IOO] > 0) set_vert(X, i, j, k);
          if (cu[OIO] > 0) set_vert(Y, i, j, k);
          if (cu[OOI] > 0) set_vert(Z, i, j, k);
        } else {
          if (cu[IOO] < 0) set_vert(X, i, j, k);
          if (cu[OIO] < 0) set_vert(Y, i, j, k);
          if (cu[OOI] < 0) set_vert(Z, i, j, k);
...
static int test_face(int face) {
  ...
  switch (face) {
    ...
    default:
      fprintf(
          stderr, "%s:%d: Invalid face code %d\n", __FILE__, __LINE__, face);
      exit(2);
  };
  ...
  switch (q->Case) {
    ...
    case 10:
      ...
      b = cu[IIO] * (cu[OOI] - cu[OOO]) + cu[OOO] * (cu[III] - cu[IIO]) -
          cu[IOO] * (cu[OII] - cu[OIO]) - cu[OIO] * (cu[IOI] - cu[IOO]);
      t = -b / (2 * a);
    ...
    case 13:
      ...
      switch (edge) {
        ...
        case 11:
          ...
          break;
        default:
          fprintf(stderr, "%s:%d: Invalid edge %d\n", __FILE__, __LINE__, edge);
          exit(2);
      }
      break;
    default:
      fprintf(
          stderr, "%s:%d: Invalid ambiguous case %d\n", __FILE__, __LINE__,
          q->Case);
      exit(2);
  }
  if (At >= 0) test++;
...
static int process_cu(void) {
  ...
  switch (q->Case) {
    ...
    case 6:
      ...
      break;
    case 7:
      if (test_face(test7[q->config][0])) q->subconfig += 1;
      if (test_face(test7[q->config][1])) q->subconfig += 2;
      if (test_face(test7[q->config][2])) q->subconfig += 4;
      switch (q->subconfig) {
        case 0:
    ...
    case 12:
      ...
      break;
    case 13:
      if (test_face(test13[q->config][0])) q->subconfig += 1;
      if (test_face(test13[q->config][1])) q->subconfig += 2;
      if (test_face(test13[q->config][2])) q->subconfig += 4;
      if (test_face(test13[q->config][3])) q->subconfig += 8;
      if (test_face(test13[q->config][4])) q->subconfig += 16;
      if (test_face(test13[q->config][5])) q->subconfig += 32;
      switch (subconfig13[q->subconfig]) {
        ...
        default:
          fprintf(
              stderr, "%s:%d: Invalid ambiguous case 13?\n", __FILE__,
              __LINE__);
          exit(2);
      }
      break;
...
static int add_tri3(const int* trig, int n, int v12) {
  ...
  for (t = 0; t < 3 * n; t++) {
    ...
    if (tv[t % 3] == -1) {
      fprintf(
          stderr, "%s:%d: Marching Cubes: invalid triangle %d\n", __FILE__,
          __LINE__, q->nt + 1);
      exit(1);
    }
    if (t % 3 == 2) {
