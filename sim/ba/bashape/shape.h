...
static void CreateFieldS(Shape b, scalar c) {
  foreach() {
    double h = Delta;
    c[] = vof_3d((b.x - x)/h , (b.y - y)/h, (b.z - z)/h, b.rx/h);
  }
}
static int GoodS(Shape b, double x, double y, double z, double h) {
  return 1;
}
static void CreateFieldC(Shape b, scalar c) {
  foreach() {
    double h = Delta;
...
static int GoodC(Shape b, double x, double y, double z, double h) {
  double d, p;
  x -= b.x;
  y -= b.y;
  z -= b.z;
