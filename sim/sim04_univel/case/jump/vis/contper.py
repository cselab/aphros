...
per = False # translate by 0.5 and apply periodic
if av[1] == '-per':
    av = [av[0]] + av[2:]
    per = True
...
if per:
    renderView1.CenterOfRotation[0] += 0.5
    renderView1.CameraPosition[0] += 0.5
    renderView1.CameraFocalPoint[0] += 0.5
...
if per:
    ...
    cl.ClipType = 'Box'
    cl.Scalars = ['CELLS', 'c']
    cl.ClipType.Bounds = [0., 1., 0., 0.45, 0.0, 0.45]
    svtk = cl
    tr = Transform(Input=svtk)
    tr.Transform = 'Transform'
    tr.Transform.Translate = [1.0, 0.0, 0.0]
    ap = AppendDatasets(Input=[svtk, tr])
    svtk = ap
...
if per:
    clip1.ClipType.Bounds = [0.5, 1.5, 0., 0.45, 0.0, 0.45]
else:
