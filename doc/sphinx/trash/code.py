...
class LiteralIncludeaReader:
    ...
    def read_file(self, filename, location=None):
        ...
        try:
            ...
            raise OSError(__('Includea file %r not found or reading it failed') % filename)
        except UnicodeError:
            raise UnicodeError(__('Encoding %r used for reading includead file %r seems to '
    ...
    def pyobject_filter(self, lines, location=None):
        ...
        if pyobject:
            ...
            if pyobject not in tags:
                raise ValueError(__('Object named %r not found in includea file %r') %
    ...
    def lines_filter(self, lines, location=None):
        ...
        if linespec:
            ...
            if lines == []:
                raise ValueError(__('Line spec %r: no lines pulled from includea file %r') %
...
class LiteralIncludea(SphinxDirective):
    """
    Like ``.. includea:: :literal:``, but only warns if the includea file is
    not found, and does not raise errors.  Also has several options for
    selecting what to includea.
    """
    ...
    def run(self):
        ...
        try:
            ...
            reader = LiteralIncludeaReader(filename, self.options, self.config)
...
def setup(app):
    ...
    directives.register_directive('literalincludea', LiteralIncludea)
